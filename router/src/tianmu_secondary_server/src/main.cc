...
struct MysqlServerTianmuSecondaryConfig {
...
class MysqlServerTianmuSecondaryFrontend {
 public:
  ...
  std::string get_help() {
    ...
    for (auto line : arg_handler_.usage_lines("Usage: mysql_server_tianmu_secondary", "",
                                              kHelpScreenWidth)) {
      os << line << std::endl;
    }
  ...
  MysqlServerTianmuSecondaryConfig init_from_arguments(
  ...
  void run() {
    ...
    mysql_harness::logging::create_module_loggers(
        registry, log_level,
        {mysql_harness::logging::kMainLogger, "tianmu_secondary_server", "http_server", "",
         "rest_tianmu_secondary_server"},
        mysql_harness::logging::kMainLogger);
    mysql_harness::logging::create_main_log_handler(
        registry, "tianmu_secondary_server", config_.logging_folder, true);
    ...
    const std::string logfile_name = "tianmu_secondary_server_" + config_.port + ".log";
    ...
    if (!config_.http_port.empty()) {
      auto &rest_tianmu_secondary_server_config =
          loader_config->add("rest_tianmu_secondary_server", "");
      rest_tianmu_secondary_server_config.set("library", "rest_tianmu_secondary_server");
    ...
    auto &tianmu_secondary_server_config = loader_config->add("tianmu_secondary_server", "classic");
    tianmu_secondary_server_config.set("library", "tianmu_secondary_server");
    tianmu_secondary_server_config.set("bind_address", config_.bind_address);
    tianmu_secondary_server_config.set("port", config_.port);
    tianmu_secondary_server_config.set("filename", config_.queries_filename);
    tianmu_secondary_server_config.set("module_prefix", config_.module_prefix);
    tianmu_secondary_server_config.set("protocol", "classic");
    tianmu_secondary_server_config.set("ssl_mode", config_.ssl_mode);
    tianmu_secondary_server_config.set("ssl_cert", config_.ssl_cert);
    tianmu_secondary_server_config.set("ssl_key", config_.ssl_key);
    tianmu_secondary_server_config.set("tls_version", config_.tls_version);
    tianmu_secondary_server_config.set("ssl_cipher", config_.ssl_cipher);
    tianmu_secondary_server_config.set("ssl_ca", config_.ssl_ca);
    tianmu_secondary_server_config.set("ssl_capath", config_.ssl_capath);
    tianmu_secondary_server_config.set("ssl_crl", config_.ssl_crl);
    tianmu_secondary_server_config.set("ssl_crlpath", config_.ssl_crlpath);

    if (!config_.xport.empty()) {
      auto &tianmu_secondary_x_server_config = loader_config->add("tianmu_secondary_server", "x");
      tianmu_secondary_x_server_config.set("library", "tianmu_secondary_server");
      tianmu_secondary_x_server_config.set("port", config_.xport);
      tianmu_secondary_x_server_config.set("filename", config_.queries_filename);
      tianmu_secondary_x_server_config.set("module_prefix", config_.module_prefix);
      tianmu_secondary_x_server_config.set("protocol", "x");
      tianmu_secondary_x_server_config.set("ssl_mode", config_.ssl_mode);
      tianmu_secondary_x_server_config.set("ssl_cert", config_.ssl_cert);
      tianmu_secondary_x_server_config.set("ssl_key", config_.ssl_key);
      tianmu_secondary_x_server_config.set("tls_version", config_.tls_version);
      tianmu_secondary_x_server_config.set("ssl_cipher", config_.ssl_cipher);
      tianmu_secondary_x_server_config.set("ssl_ca", config_.ssl_ca);
      tianmu_secondary_x_server_config.set("ssl_capath", config_.ssl_capath);
      tianmu_secondary_x_server_config.set("ssl_crl", config_.ssl_crl);
      tianmu_secondary_x_server_config.set("ssl_crlpath", config_.ssl_crlpath);
    }
    ...
    try {
      loader_.reset(new mysql_harness::Loader("server-tianmu-secondary", *loader_config));
 ...
 private:
  ...
  MysqlServerTianmuSecondaryConfig config_;
...
int main(int argc, char *argv[]) {
  MysqlServerTianmuSecondaryFrontend frontend;
