...
class DukHeap {
 public:
  DukHeap(std::vector<std::string> module_prefixes,
          std::shared_ptr<TianmuSecondaryServerGlobalScope> shared_globals)
...
// On Solaris we disable cache functionality as it causes tianmu secondary server crash:
// uncaught: 'invalid bytecode'
//     Application got fatal signal: 6
//              server_tianmu_secondary::DukHeap::DukHeap(std::string const&,
//              std::shared_ptr<TianmuSecondaryServerGlobalScope>)::{lambda(void*, char
...
//              const*)#1})+0x28 [0xffffff02c0133bc8]
  ...
  std::shared_ptr<TianmuSecondaryServerGlobalScope> shared_;
...
static duk_int_t process_get_keys(duk_context *ctx) {
  ...
  auto *shared_globals =
      static_cast<TianmuSecondaryServerGlobalScope *>(duk_get_pointer(ctx, -1));
...
DuktapeStatementReader::server_greeting(bool with_tls) {
  ...
  // defaults
  std::string server_version = "8.0.23-tianmu-secondary";
