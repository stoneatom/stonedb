...
#include "mysqlrouter/tianmu_secondary_server_component.h"
...
static constexpr const char kSectionName[]{"rest_tianmu_secondary_server"};
static constexpr const char kRestGlobalsUri[]{"^/api/v1/tianmu_secondary_server/globals/$"};
...
class RestApiV1TianmuSecondaryServerGlobals : public BaseRequestHandler {
 public:
  RestApiV1TianmuSecondaryServerGlobals() : last_modified_(time(nullptr)) {}
 ...
 private:
  ...
  void handle_global_put_all(HttpRequest &req) {
    ...
    typename TianmuSecondaryServerGlobalScope::type all_globals;
    ...
    auto shared_globals =
        TianmuSecondaryServerComponent::get_instance().get_global_scope();
...
class RestApiV1TianmuSecondaryServerConnections : public BaseRequestHandler {
 public:
 ...
 private:
  ...
  void handle_connections_delete_all(HttpRequest &req) {
    // tell the tianmu_secondary_server to close all connections
    TianmuSecondaryServerComponent::get_instance().close_all_connections();
...
static void run(mysql_harness::PluginFuncEnv *env) {
  ...
  srv.add_route(kRestGlobalsUri,
                std::make_unique<RestApiV1TianmuSecondaryServerGlobals>());
  Scope_guard global_route_guard(
      [&srv]() { srv.remove_route(kRestGlobalsUri); });

  srv.add_route(kRestConnectionsUri,
                std::make_unique<RestApiV1TianmuSecondaryServerConnections>());
...
#if defined(_MSC_VER) && defined(rest_tianmu_secondary_server_EXPORTS)
...
static const std::array<const char *, 2> plugin_requires = {
    "logger",
    // "tianmu_secondary_server",
    "http_server",
};

extern "C" {
mysql_harness::Plugin DLLEXPORT harness_plugin_rest_tianmu_secondary_server = {
