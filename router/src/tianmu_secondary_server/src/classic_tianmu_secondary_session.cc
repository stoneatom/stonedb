...
#include "classic_tianmu_secondary_session.h"
...
void MySQLServerTianmuSecondarySessionClassic::server_greeting() {
...
void MySQLServerTianmuSecondarySessionClassic::client_greeting() {
...
void MySQLServerTianmuSecondarySessionClassic::auth_switched() {
  // check we have enough of a client-greeting.
  std::vector<uint8_t> payload;
  auto frame_decode_res = protocol_.read_packet(payload);
  if (!frame_decode_res) {
...
void MySQLServerTianmuSecondarySessionClassic::send_response_then_disconnect() {
...
void MySQLServerTianmuSecondarySessionClassic::send_response_then_idle() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerTianmuSecondarySessionClassic::idle() {
  std::vector<uint8_t> payload;
  auto frame_decode_res = protocol_.read_packet(payload);
  if (!frame_decode_res) {
    auto ec = frame_decode_res.error();
  ...
  switch (cmd) {
    ...
    case classic_protocol::Codec<
      ...
      // TIME_WAIT on the tianmu-secondary-server.
...
void MySQLServerTianmuSecondarySessionClassic::finish() { disconnect(); }

void MySQLServerTianmuSecondarySessionClassic::run() { server_greeting(); }
...
bool MySQLServerTianmuSecondarySessionClassic::authenticate(
