...
#include "mysql_server_tianmu_secondary.h"
...
#include "tianmu_secondary_session.h"
...
MySQLServerTianmuSecondary::MySQLServerTianmuSecondary(net::io_context &io_ctx,
...
void MySQLServerTianmuSecondary::close_all_connections() {
...
class Acceptor {
 public:
  ...
  Acceptor(net::io_context &io_ctx, std::string protocol_name,
           WaitableMonitor<std::list<std::unique_ptr<MySQLServerTianmuSecondarySession>>>
               &client_sessions,
  ...
  void accepted(protocol_type::socket client_sock) {
    ...
    auto session_it = client_sessions_([&](auto &socks) {
      if (protocol_name_ == "classic") {
        socks.emplace_back(std::make_unique<MySQLServerTianmuSecondarySessionClassic>(
            MySQLClassicProtocol{std::move(client_sock), client_ep_,
                                 tls_server_ctx_},
            std::move(reader), false, with_tls_));
      } else {
        socks.emplace_back(std::make_unique<MySQLServerTianmuSecondarySessionX>(
...
void MySQLServerTianmuSecondary::run(mysql_harness::PluginFuncEnv *env) {
