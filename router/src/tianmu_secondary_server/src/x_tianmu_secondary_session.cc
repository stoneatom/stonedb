...
*/
#include "x_tianmu_secondary_session.h"
...
#include "router/src/tianmu_secondary_server/src/statement_reader.h"
...
MySQLServerTianmuSecondarySessionX::clock_type::time_point
MySQLServerTianmuSecondarySessionX::notice_expiry() const {
...
bool MySQLServerTianmuSecondarySessionX::encode_due_async_notices(
    const MySQLServerTianmuSecondarySessionX::clock_type::time_point &start_time) {
...
MySQLServerTianmuSecondarySessionX::MySQLServerTianmuSecondarySessionX(
    MySQLXProtocol protocol,
    std::unique_ptr<StatementReaderBase> statement_processor,
    const bool debug_mode, bool with_tls)
    : MySQLServerTianmuSecondarySession(std::move(statement_processor), debug_mode),
      async_notices_(this->json_reader_->get_async_notices()),
      protocol_{std::move(protocol)},
      with_tls_{with_tls} {}

void MySQLServerTianmuSecondarySessionX::send_response_then_handshake() {
...
void MySQLServerTianmuSecondarySessionX::greeting() {
...
void MySQLServerTianmuSecondarySessionX::handshake() {
  std::vector<uint8_t> payload;
...
void MySQLServerTianmuSecondarySessionX::send_response_then_disconnect() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerTianmuSecondarySessionX::send_response_then_first_idle() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerTianmuSecondarySessionX::send_response_then_idle() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerTianmuSecondarySessionX::send_notice_then_notices() {
  protocol_.async_send([&, to_send = protocol_.send_buffer().size()](
                           std::error_code ec, size_t transferred) {
    if (ec) {
      if (ec != std::errc::operation_canceled) {
...
void MySQLServerTianmuSecondarySessionX::notices() {
...
void MySQLServerTianmuSecondarySessionX::idle() {
...
void MySQLServerTianmuSecondarySessionX::run() { greeting(); }
