...
#include "tianmu_secondary_server_plugin.h"
...
static constexpr const char kSectionName[]{"tianmu_secondary_server"};
...
//
// - one per tianmu-secondary-server instance
...
static void start(mysql_harness::PluginFuncEnv *env) {
  ...
  try {
    ...
    auto srv = std::make_shared<server_tianmu_secondary::MySQLServerTianmuSecondary>(
    ...
    TianmuSecondaryServerComponent::get_instance().register_server(key, srv);
...
extern "C" {
mysql_harness::Plugin TIANMU_SECONDARY_SERVER_EXPORT harness_plugin_tianmu_secondary_server = {
    mysql_harness::PLUGIN_ABI_VERSION,       // abi-version
    mysql_harness::ARCHITECTURE_DESCRIPTOR,  // arch
    "Tianmu Secondary MySQL Server for testing",         // name
