...
class VirtualColumnBase : public core::Column {
 public:
  struct VarMap {
    VarMap(core::VarID v, core::JustATable *t, int d)
        : var_(v), col_idx_(v.col < 0 ? -v.col - 1 : v.col), dimension_(d), table_(t->shared_from_this()) {}

    core::VarID var_;  // variable identified by table alias and column number
    int col_idx_;      // column index (not negative)
    int dimension_;    // dimension in multiindex corresponding to the alias

    core::JustATable *tabptr_;
    std::shared_ptr<core::JustATable> GetTabPtr() const { return table_.lock(); }
    bool operator<(VarMap const &v) const {
      return (var_ < v.var_) || ((var_ == v.var_) && (col_idx_ < v.col_idx_)) ||
             ((var_ == v.var_) && (col_idx_ == v.col_idx_) && (GetTabPtr() < v.GetTabPtr())) ||
             ((var_ == v.var_) && (col_idx_ == v.col_idx_) && (GetTabPtr() == v.GetTabPtr()) && (dimension_ < v.dimension_));
    }
    std::weak_ptr<core::JustATable> table_;  // table corresponding to the alias
  };
  ...
  virtual void SetParamTypes([[maybe_unused]] core::MysqlExpression::TypeOfVars *types){};
   ...
   *
   */
  virtual int GetDim() { return dimension_; }
  core::Transaction *ConnInfo() { return txn_info_; }
  core::MultiIndex *GetMultiIndex() { return multi_idx_; }
  ...
  void SetLocalDistVals(int64_t loc_dist_vals) {
    if (vc_distinct_vals_ == common::NULL_VALUE_64 || vc_distinct_vals_ > loc_dist_vals) 
       vc_distinct_vals_ = loc_dist_vals;
  }
 ...
 protected:
  core::MultiIndex *multi_idx_;
  core::Transaction *txn_info_;
  ...
  //do evaluation at the first rough set.
  mutable bool is_first_eval_;

  int dimension_;  // valid only for SingleColumn and for ExpressionColumn if based on
                   // a single table only
  ...
  int64_t vc_distinct_vals_;  // upper approximation of non-null distinct vals, or
