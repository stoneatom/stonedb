...
class MappedCircularBuffer {
 ...
 public:
  ...
  MappedCircularBuffer(const std::string &file, size_t sz_MB)
    ...
    if (sb.st_size == 0) {
      ...
      STONEDB_LOG(LogCtl_Level::INFO, "created delayed buffer file %s with size %ldMB", file.c_str(), buf_size_ / 1_MB);
    } else {
      // file already exists
      if (sb.st_size == static_cast<decltype(sb.st_size)>(FileSize())) {
        file_exists = true;
        STONEDB_LOG(LogCtl_Level::INFO, "use delayed buffer file %s with size %ldMB", file.c_str(), buf_size_ / 1_MB);
      } else {
 ...
 private:
  void MapFile(const std::string &file) {
    ...
    STONEDB_LOG(LogCtl_Level::INFO, "insert buffer address %p", start_addr_);
  }
