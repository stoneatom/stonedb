...
void log_setup(const std::string &log_file) { stonedblog.addOutput(new system::FileOut(log_file)); }
...
logger::LogCtl_Level LogCtl::GetGlobalLevel() {
  return static_cast<logger::LogCtl_Level>(stonedb_sysvar_global_debug_level);
}
...
#if 0
    if (current_tx == nullptr) {
        //the session is not setup, will use stonedb_sysvar_global_debug_level only
...
void LogCtl::LogMsg(logger::LogCtl_Level level, const char *file, int line, const char *format, ...) {
  ...
  if (LogEnabled(level)) {
    ...
    buff[length] = '\0';
    stonedblog << system::lock << "[" << logger::get_level_str(level) << "] [" << basename(file) << ":" << line
...
void LogCtl::LogMsg(logger::LogCtl_Level level, const std::string &msg) {
  if (LogEnabled(level)) {
    stonedblog << system::lock << logger::get_level_str(level) << " MSG: " << msg << system::unlock;
  }
}
...
void LogCtl::LogMsg(logger::LogCtl_Level level, std::stringstream &msg_ss) {
  if (LogEnabled(level)) {
    stonedblog << system::lock << logger::get_level_str(level) << " MSG: " << msg_ss.str() << system::unlock;
...
void stonedb_log(enum loglevel mysql_level, const char *buffer, [[maybe_unused]] size_t length) {
