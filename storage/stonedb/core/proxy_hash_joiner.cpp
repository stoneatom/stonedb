    ...
    if (slice_capability_.type != MIIterator::SliceCapability::Type::kDisable) {
      if (slice_capability_.type == MIIterator::SliceCapability::Type::kLinear) {
        if (stonedb_sysvar_async_join_setting.pack_per_step > 0)
          ...
          DEBUG_ASSERT(stonedb_sysvar_async_join_setting.rows_per_step > 0);
          slice_type_ = base::sprint("per %d rows", stonedb_sysvar_async_join_setting.rows_per_step);
        }
      } else {
        slice_type_ = "fixed";
      }
  ...
  std::shared_ptr<MIIterator> Poll() {
    ...
    if (slice_capability_.type != MIIterator::SliceCapability::Type::kDisable) {
      ...
      if (slice_capability_.type == MIIterator::SliceCapability::Type::kLinear) {
        ...
        if (stonedb_sysvar_async_join_setting.pack_per_step > 0) {
          pack_iter.reset(new MIPackStepIterator(cur_pos_, stonedb_sysvar_async_join_setting.pack_per_step, *miter_));
          ...
          pack_iter.reset(
              new MIRowsStepIterator(cur_pos_, stonedb_sysvar_async_join_setting.rows_per_step, origin_size_, *miter_));
...
class ProxyHashJoiner::Action {
 ...
 private:
  ...
  void InitHashTables(std::vector<int> &&hash_table_key_size, std::vector<ColumnBinEncoder> &&column_encoder) {
    ...
    if (stonedb_sysvar_async_join_setting.traverse_slices > 0)
      hash_table_count = std::min(hash_table_count, stonedb_sysvar_async_join_setting.traverse_slices);
    ...
    tables_manager_.reset(JoinThreadTableManager::CreateSharedTableManager(
        ...
        force_switching_sides_ || stonedb_sysvar_join_disable_switch_side));
  }
  ...
  base::future<> MatchDim(int64_t wait_matched_rows) {
    ...
    if (stonedb_sysvar_async_join_setting.match_slices > 0)
      slice_count = std::min<size_t>(slice_count, stonedb_sysvar_async_join_setting.match_slices);
