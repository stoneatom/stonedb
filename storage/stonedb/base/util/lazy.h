 ...
 */
#ifndef STONEDB_BASE_LAZY_H_
#define STONEDB_BASE_LAZY_H_
#pragma once
...
///
/// The instance may be created only using base::value_of helper function.
///
...
/// Create a base::lazy_eval object that will use a given functor for
...
/// Create a base::lazy_deref_wrapper object.
///
...
/// Output operator for a base::lazy_eval<Func>
/// This would allow printing a base::lazy_eval<Func> as if it's a regular
/// value.
///
/// For example:
///
/// `logger.debug("heavy eval result:{}", base::value_of([&] { return <heavy
/// evaluation>; }));`
///
/// (If a logging level is lower than "debug" the evaluation will not take
/// place.)
///
...
/// \return os
template <typename Func>
ostream &operator<<(ostream &os, const base::lazy_eval<Func> &lf) {
  return os << lf();
}

template <typename Func>
ostream &operator<<(ostream &os, base::lazy_eval<Func> &lf) {
  return os << lf();
}
...
template <typename T>
ostream &operator<<(ostream &os, base::lazy_deref_wrapper<T> ld) {
  if (ld.p) {
    return os << *ld.p;
  }
