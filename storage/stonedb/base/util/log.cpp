...
static void print_no_timestamp([[maybe_unused]] std::ostream &os) {}
...
static inline timestamp_tag space_and_current_timestamp() { return timestamp_tag{}; }
...
std::ostream &operator<<(std::ostream &out, log_level level) { return out << log_level_names.at(level); }
...
logger::logger(sstring name) : _name(std::move(name)) { global_logger_registry().register_logger(this); }
...
logger::~logger() { global_logger_registry().unregister_logger(this); }

void logger::really_do_log(log_level level, const char *fmt, stringer **s, size_t n) {
...
void logger::set_stdout_enabled(bool enabled) { _stdout.store(enabled, std::memory_order_relaxed); }

void logger::set_syslog_enabled(bool enabled) { _syslog.store(enabled, std::memory_order_relaxed); }

bool logger::is_shard_zero() { return engine().cpu_id() == 0; }
  ...
  for (const auto &pair : s.logger_levels) {
    try {
      ...
      throw std::runtime_error(
          base::sprint("Unknown logger '%s'. Use --help-loggers to list available loggers.", pair.first));
    }
  }
...
sstring level_name(log_level level) { return log_level_names.at(level); }
...
logging_settings create_default_logging_settings() {
  ...
  // \"NAME0=LEVEL0[:NAME1=LEVEL1:...]\". " "Valid logger names can be queried
  // with --help-logging. " "Valid values for levels are trace, debug, info,
  // warn, error. " "This option can be specified multiple times."
  program_options::string_map raw_levels;
  std::unordered_map<sstring, log_level> levels;
  parse_logger_levels(raw_levels, std::inserter(levels, levels.begin()));

  return logging_settings{
      std::move(levels),
      parse_log_level(sstring("info")),  // Default log level for log messages. Valid values
                                         // are trace, debug, info, warn, error.
      true,                              // Send log output to stdout.
      false,                             // Send log output to syslog.
      logger_timestamp_style::real       // Select timestamp style for stdout logs:
                                         // none|boot|real
  };
}
...
namespace boost {
template <>
stonedb::base::log_level lexical_cast(const std::string &source) {
  std::istringstream in(source);
  stonedb::base::log_level level;
  if (!(in >> level)) {
...
std::ostream &operator<<(std::ostream &out, const std::exception_ptr &eptr) {
  ...
  try {
    ...
    if (tp) {
      out << stonedb::base::pretty_type_name(*tp);
    } else {
...
std::ostream &operator<<(std::ostream &out, const std::exception &e) {
  return out << stonedb::base::pretty_type_name(typeid(e)) << " (" << e.what() << ")";
}

std::ostream &operator<<(std::ostream &out, const std::system_error &e) {
  return out << stonedb::base::pretty_type_name(typeid(e)) << " (error " << e.code() << ", " << e.code().message()
             << ")";
}
