 ...
 */
#ifndef STONEDB_BASE_NONCOPYABLE_FUNCTION_H_
#define STONEDB_BASE_NONCOPYABLE_FUNCTION_H_
#pragma once
...
class noncopyable_function<Ret(Args...)> {
 ...
 private:
  static Ret empty_call(const noncopyable_function *func, Args... args) { throw std::bad_function_call(); }
  ...
  struct direct_vtable_for {
    ...
    static void destroy(noncopyable_function *func) { access(func)->~Func(); }
    static void initialize(Func &&from, noncopyable_function *to) { new (access(to)) Func(std::move(from)); }
  ...
  struct indirect_vtable_for {
    ...
    static void destroy(noncopyable_function *func) { delete access(func); }
    static void initialize(Func &&from, noncopyable_function *to) { to->_storage.indirect = new Func(std::move(from)); }
 ...
 public:
  ...
  noncopyable_function(Func func) noexcept {
    // If we want, we can support throwing move constructible functions by using
    // the indirect path
  ...
  template <typename Object, typename... AllButFirstArg>
  noncopyable_function(Ret (Object::*member)(AllButFirstArg...) const) noexcept
      : noncopyable_function(std::mem_fn(member)) {}

  ~noncopyable_function() { _vtable->destroy(this); }
  ...
  Ret operator()(Args... args) const { return _vtable->call(this, std::forward<Args>(args)...); }
};
...
#endif  // STONEDB_BASE_NONCOPYABLE_FUNCTION_H_
