 ...
 */
#ifndef STONEDB_BASE_CORE_BYTEORDER_H_
#define STONEDB_BASE_CORE_BYTEORDER_H_
#pragma once
...
template <typename T>
inline void write_le(char *p, T datum) {
...
template <typename T>
inline void write_be(char *p, T datum) {
  datum = cpu_to_be(datum);
  std::copy_n(reinterpret_cast<const char *>(&datum), sizeof(T), p);
}

template <typename T>
inline T consume_be(const char *&p) {
...
template <typename T>
inline void produce_be(char *&p, T datum) {
  write_be<T>(p, datum);
  p += sizeof(T);
}
...
#endif  // STONEDB_BASE_CORE_BYTEORDER_H_
