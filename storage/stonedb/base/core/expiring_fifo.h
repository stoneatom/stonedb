 ...
 */
#ifndef STONEDB_BASE_EXPIRING_FIFO_H_
#define STONEDB_BASE_EXPIRING_FIFO_H_
#pragma once
...
///
/// OnExpiry is a functor which will be called with a reference to T right
/// before it expires. T is removed and destroyed from the container immediately
/// after OnExpiry returns. OnExpiry callback must not modify the container, it
/// can only modify its argument.
///
...
class expiring_fifo {
 ...
 private:
  ...
  // Ensures that front() is not expired by dropping expired elements from the
  // front.
  ...
  */
  ...
  T &front() { return *_list.front().payload; }

  /// Returns a reference to the element in the front.
  /// Valid only when !empty().
  const T &front() const { return *_list.front().payload; }

  /// Returns the number of elements contained.
  ///
  /// \note Expired elements are not contained. Expiring element is still
  /// contained when OnExpiry is called.
  size_t size() const { return _size; }

  /// Reserves storage in the container for at least 'size' elements.
  /// Note that expired elements may also take space when they are not in the
  /// front of the queue.
  ///
  /// Doesn't give any guarantees about exception safety of subsequent
  /// push_back().
  void reserve(size_t size) { return _list.reserve(size); }
  ...
  /// Adds element to the back of the queue.
  /// The element will expire when timeout is reached, unless it is
  /// time_point::max(), in which case it never expires.
...
#endif  // STONEDB_BASE_EXPIRING_FIFO_H_
