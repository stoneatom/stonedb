 ...
 */
#ifndef STONEDB_BASE_PIPE_H_
#define STONEDB_BASE_PIPE_H_
#pragma once
...
/// Repeat a blocking task, then exit              | \ref repeat(), \ref
/// do_until() Provide mutual exclusion between two tasks     | \ref semaphore,
/// \ref shared_mutex Pass a stream of data between two fibers       | \ref
/// base::pipe Safely shut down a resource                    | \ref base::gate,
/// \ref base::with_gate() Hold on to an object while a fiber is running  | \ref
/// do_with()
///
...
class broken_pipe_exception : public std::exception {
 public:
  virtual const char *what() const noexcept { return "Broken pipe"; }
};

class unread_overflow_exception : public std::exception {
 public:
  virtual const char *what() const noexcept { return "pipe_reader::unread() overflow"; }
};
...
class pipe_buffer {
 ...
 public:
  ...
  future<std::experimental::optional<T>> read() { return _buf.pop_eventually(); }
  future<> write(T &&data) { return _buf.push_eventually(std::move(data)); }
  bool readable() const { return _write_open || !_buf.empty(); }
  bool writeable() const { return _read_open; }
  bool close_read() {
...
/// \brief Read side of a \ref base::pipe
///
...
/// reader/writer pair through \ref base::pipe.
template <typename T>
class pipe_reader {
 private:
 ...
 public:
  ...
  pipe_reader(pipe_reader &&other) : _bufp(other._bufp) { other._bufp = nullptr; }
  pipe_reader &operator=(pipe_reader &&other) { std::swap(_bufp, other._bufp); }
};

/// \brief Write side of a \ref base::pipe
///
...
class pipe_writer {
 ...
 public:
  ...
  pipe_writer(pipe_writer &&other) : _bufp(other._bufp) { other._bufp = nullptr; }
  pipe_writer &operator=(pipe_writer &&other) { std::swap(_bufp, other._bufp); }
};
