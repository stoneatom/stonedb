 ...
 */
#ifndef STONEDB_BASE_LOWRES_CLOCK_H_
#define STONEDB_BASE_LOWRES_CLOCK_H_
#pragma once
...
class lowres_clock_impl final {
 public:
  ...
  // The clocks' resolutions are 10 ms. However, to make it is easier to do
  // calculations with `std::chrono::milliseconds`, we make the clock period 1
  // ms instead of 10 ms.
 ...
 private:
  // Both counters are updated by cpu0 and read by other cpus. Place them on
  // their own cache line to avoid false sharing.
  struct alignas(base::cache_line_size) counters final {
...
///
/// This is a monotonic clock with a granularity of 10 ms. Time points from this
/// clock do not correspond to system time.
///
/// The primary benefit of this clock is that invoking \c now() is inexpensive
/// compared to \c std::chrono::steady_clock::now().
///
/// \see \c lowres_system_clock for a low-resolution clock which produces time
/// points corresponding to system time.
///
class lowres_clock final {
 public:
  ...
  ///
  static time_point now() { return lowres_clock_impl::steady_now(); }
};

///
/// \brief Low-resolution and efficient system clock.
///
/// This clock has the same granularity as \c lowres_clock, but it is not
/// required to be monotonic and its time points correspond to system time.
///
/// The primary benefit of this clock is that invoking \c now() is inexpensive
/// compared to \c std::chrono::system_clock::now().
///
class lowres_system_clock final {
 public:
  ...
  /// \note Outside of a Seastar application, the result is undefined.
  ///
  static time_point now() { return lowres_clock_impl::system_now(); }
...
#endif  // STONEDB_BASE_LOWRES_CLOCK_H_
