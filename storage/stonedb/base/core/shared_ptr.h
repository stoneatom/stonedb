...
#ifndef STONEDB_BASE_SHARED_PTR_H_
#define STONEDB_BASE_SHARED_PTR_H_
#pragma once
...
namespace std {
template <typename T>
struct hash<stonedb::base::lw_shared_ptr<T>> : private hash<T *> {
  size_t operator()(const stonedb::base::lw_shared_ptr<T> &p) const { return hash<T *>::operator()(p.get()); }
};

template <typename T>
struct hash<stonedb::base::shared_ptr<T>> : private hash<T *> {
  size_t operator()(const stonedb::base::shared_ptr<T> &p) const { return hash<T *>::operator()(p.get()); }
};
}  // namespace std
...
#endif  // STONEDB_BASE_SHARED_PTR_H_
