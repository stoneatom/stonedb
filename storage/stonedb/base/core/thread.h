...
#ifndef STONEDB_BASE_THREAD_H_
#define STONEDB_BASE_THREAD_H_
#pragma once
...
///
/// Like other seastar code, seastar threads may not issue blocking system
/// calls.
///
/// A seastar thread blocking point is any function that returns a \ref
/// future<>. you block by calling \ref future<>::get(); this waits for the
/// future to become available, and in the meanwhile, other seastar threads and
/// seastar non-threaded code may execute.
///
/// Example:
/// \code
///    base::thread th([] {
...
/// result from this execution is by using the \ref base::async() function.
...
class thread_attributes {
 public:
  ...
  stdx::optional<base::scheduling_group> sched_group;
};
...
class thread_context {
  ...
  using all_thread_list = boost::intrusive::list<
      thread_context,
      boost::intrusive::member_hook<thread_context, boost::intrusive::list_member_hook<>, &thread_context::_all_link>,
      boost::intrusive::constant_time_size<false>>;
 ...
 private:
  static void s_main(int lo,
                     int hi);  // all parameters MUST be 'int' for makecontext
  void setup();
  void main();
...
class thread {
 ...
 public:
  ...
  ///
  /// Waits for thread execution to terminate, and marks the thread object as
  /// not representing a running thread of execution.
  future<> join();
  ...
  static bool running_in_thread() { return thread_impl::get() != nullptr; }
...
/// An instance of this class can be used to assign a thread to a particular
/// scheduling group. Threads can share the same scheduling group if they hold a
/// pointer to the same instance of this class.
///
/// All threads that belongs to a scheduling group will have a time granularity
/// defined by \c period, and can specify a fraction \c usage of that period
/// that indicates the maximum amount of time they expect to run. \c usage, is
/// expected to be a number between 0 and 1 for this to have any effect. Numbers
/// greater than 1 are allowed for simplicity, but they just have the same
/// meaning of 1, alas, "the whole period".
///
/// Note that this is not a preemptive runtime, and a thread will not exit the
/// CPU unless it is scheduled out. In that case, \c usage will not be enforced
/// and the thread will simply run until it loses the CPU. This can happen when
/// a thread waits on a future that is not ready, or when it voluntarily call
/// yield.
///
/// Unlike what happens for a thread that is not part of a scheduling group -
/// which puts itself at the back of the runqueue everytime it yields, a thread
/// that is part of a scheduling group will only yield if it has exhausted its
/// \c usage at the call to yield. Therefore, threads in a schedule group can
/// and should yield often.
///
/// After those events, if the thread has already run for more than its
/// fraction, it will be scheduled to run again only after \c period completes,
/// unless there are no other tasks to run (the system is idle)
class thread_scheduling_group {
 ...
 public:
  ...
  /// \param usage which fraction of the \c period to assign for the scheduling
  /// group. Expected between 0 and 1.
  thread_scheduling_group(std::chrono::nanoseconds period, float usage);
  /// \brief changes the current maximum usage per period
  ///
  /// \param new_usage The new fraction of the \c period (Expected between 0 and
  /// 1) during which to run
...
inline futurize_t<std::result_of_t<std::decay_t<Func>(std::decay_t<Args>...)>> async(thread_attributes attr,
  ...
  return do_with(work{std::move(attr), std::forward<Func>(func), std::forward_as_tuple(std::forward<Args>(args)...)},
                 [](work &w) mutable {
                   ...
                   return w.th.join().then([ret = std::move(ret)]() mutable { return std::move(ret); });
                 });
}
