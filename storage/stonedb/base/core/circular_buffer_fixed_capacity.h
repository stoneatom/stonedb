 ...
 */
#ifndef STONEDB_BASE_CIRCBUFF_FIX_CAP_H_
#define STONEDB_BASE_CIRCBUFF_FIX_CAP_H_
#pragma once
...
///
/// \tparam T type of objects stored in the container; must be noexcept move
/// enabled \tparam Capacity maximum number of objects that can be stored in the
/// container; must be a power of 2
...
class circular_buffer_fixed_capacity {
 ...
 public:
  ...
  class cbiterator {
   ...
   public:
    ...
    cbiterator operator+(difference_type n) const { return cbiterator{_start, _idx + n}; }
    friend cbiterator operator+(difference_type n, cbiterator i) { return i + n; }
    cbiterator operator-(difference_type n) const { return cbiterator{_start, _idx - n}; }
    ...
    bool operator==(const cbiterator &rhs) const { return _idx == rhs._idx; }
    bool operator!=(const cbiterator &rhs) const { return _idx != rhs._idx; }
    bool operator<(const cbiterator &rhs) const { return ssize_t(_idx - rhs._idx) < 0; }
    bool operator>(const cbiterator &rhs) const { return ssize_t(_idx - rhs._idx) > 0; }
    bool operator<=(const cbiterator &rhs) const { return ssize_t(_idx - rhs._idx) <= 0; }
    bool operator>=(const cbiterator &rhs) const { return ssize_t(_idx - rhs._idx) >= 0; }
    difference_type operator-(const cbiterator &rhs) const { return _idx - rhs._idx; }
 ...
 public:
  ...
  void clear();
  iterator begin() { return iterator(_storage, _begin); }
  const_iterator begin() const { return const_iterator(_storage, _begin); }
  iterator end() { return iterator(_storage, _end); }
  const_iterator end() const { return const_iterator(_storage, _end); }
  const_iterator cbegin() const { return const_iterator(_storage, _begin); }
  const_iterator cend() const { return const_iterator(_storage, _end); }
...
template <typename T, size_t Capacity>
inline circular_buffer_fixed_capacity<T, Capacity> &circular_buffer_fixed_capacity<T, Capacity>::operator=(
    circular_buffer_fixed_capacity &&x) noexcept {
  if (this != &x) {
...
circular_buffer_fixed_capacity<T, Capacity>::erase(iterator first, iterator last) {
  ...
  // Move to the left or right depending on which would result in least amount
  // of moves. This also guarantees that iterators will be stable when removing
...
#endif  // STONEDB_BASE_CIRCBUFF_FIX_CAP_H_
