 ...
 */
#ifndef STONEDB_BASE_SCOLLECTD_H_
#define STONEDB_BASE_SCOLLECTD_H_
#pragma once
 ...
 *
 * 		scollectd::type_instance_id typ("<pluginname>",
 *"<instance_name>", "<type_name>", "<instance_name>");
 *		scollectd::add_polled_metric(typ, [<metric var> |
 *scollectd::make_typed(<data_type>, <metric_var>) [,
 *...]);
 *
 * Where
 * 	<pluginname> would be the overall 'module', e.g. "cpu"
 *  <instance_name> -> optional distinguisher between plugin instances. For cpu,
 *the built-in scollectd::per_cpu_plugin_instance constant is a good choice,
 *i.e. 0->N cpu. If there are no instances (e.g. only one), empty constant is
 *appropriate (none) <type_name> is the 'type' of metric collected, for ex.
 *"usage" (cpu/0/usage) <type_instance> is a distinguisher for metric parts of
 *the type, e.g. "idle", "user", "kernel"
 *  -> cpu/0/usage/idle | cpu/0/usage/user | cpu/0/usage/kernel
 *
 *  Each type instance can bind an arbitrary number of values, ech representing
 *some aspect in turn of the instance. The structure and interpretation is up to
 *the producer/consumer
 *
 * There is a single "scollectd" instance per cpu, and values should be bound
 *locally to this cpu. Polling is done at a frequency set in the seastar config
 *(def once per s), and all registered values will be sent via UDP packages to
 *the destination host(s)
 *
 * Note that the tuple { plugin, plugin_instance, type, type_instance } is
 *considered a unique ID for a value registration, so using the same tuple twice
 *will remove the previously registered values.
 *
 * Values can be unregistered at any time, though they must be so on the same
 *thread/cpu as they we're registered. The "registration" achor type provides
 *RAII style value unregistration semantics.
 *
 */

namespace scollectd {
extern base::logger logger;

using data_type = base::metrics::impl::data_type;
...
using plugin_id = base::metrics::group_name_type;
using plugin_instance_id = base::metrics::instance_id_type;
using type_id = base::metrics::metric_type_def;
using type_instance = base::metrics::metric_name_type;

type_id type_id_for(known_type);

using description = base::metrics::description;
...
class type_instance_id {
  ...
  void truncate(sstring &field, const char *field_desc) {
    if (field.size() > max_collectd_field_text_len) {
      ...
      sstring new_field(base::format("{}~{:d}", sstring(field.data(), max_collectd_field_text_len - suffix_len),
                                     _next_truncated_idx));
 ...
 public:
  ...
  type_instance_id(plugin_id p, plugin_instance_id pi, type_id t, scollectd::type_instance ti = std::string())
      : _plugin(std::move(p)), _plugin_instance(std::move(pi)), _type(std::move(t)), _type_instance(std::move(ti)) {
  ...
  type_instance_id(const base::metrics::impl::metric_id &id)
      : _plugin(id.group_name()),
        _plugin_instance(id.instance_id()),
        _type(id.inherit_type()),
  ...
  const plugin_id &plugin() const { return _plugin; }
  const plugin_instance_id &plugin_instance() const { return _plugin_instance; }
...
struct collectd_options {
  bool enabled = false;         // enable collectd daemon
  std::string address;          // address to send/broadcast metrics to
  unsigned poll_period = 1000;  // poll period - frequency of sending counter
                                // metrics (default: 1000ms, 0 disables)

  collectd_options() { address = "239.192.74.66:25826"; }
};

void configure(const collectd_options &options = collectd_options());
...
struct registration {
 ...
 private:
  ...
  shared_ptr<base::metrics::impl::impl> _impl;
};
...
class registrations : public std::vector<registration> {
 public:
  using vector_type = std::vector<registration>;
...
struct typed_value {
  ...
  const scollectd::type_instance &type_instance() const { return _type_instance; }
  const shared_ptr<value_list> &values() const { return _values; }
  const type_id &type() const { return _type_id; }
 ...
 */
using total_bytes = typed_value_impl<known_type::total_bytes>;
using total_connections = typed_value_impl<known_type::total_connections>;
using total_objects = typed_value_impl<known_type::total_objects>;
using total_operations = typed_value_impl<known_type::total_operations>;
using total_requests = typed_value_impl<known_type::total_requests>;
using total_sessions = typed_value_impl<known_type::total_sessions>;
using total_threads = typed_value_impl<known_type::total_threads>;
using total_time_in_ms = typed_value_impl<known_type::total_time_in_ms>;
using total_values = typed_value_impl<known_type::total_values>;
using queue_length = typed_value_impl<known_type::queue_length>;
using counter = typed_value_impl<known_type::counter>;
using count = typed_value_impl<known_type::count>;
using gauge = typed_value_impl<known_type::gauge>;
...
template <typename T>
struct is_callable<T, typename std::enable_if<!std::is_void<typename std::result_of<T()>::type>::value, void>::type>
    : public std::true_type {};

template <typename T>
struct is_callable<T, typename std::enable_if<std::is_fundamental<T>::value, void>::type> : public std::false_type {};

template <typename T>
struct data_type_for<T, typename std::enable_if<std::is_integral<T>::value && std::is_unsigned<T>::value, void>::type>
    : public std::integral_constant<data_type, data_type::COUNTER> {};
template <typename T>
struct data_type_for<T, typename std::enable_if<std::is_integral<T>::value && std::is_signed<T>::value, void>::type>
...
class value {
 public:
  ...
  struct wrap {
    wrap(const W &v) : _v(v) {}
    const W &operator()() const { return _v; }
    const W &_v;
  };
  ...
  operator data_type() const { return _type; }
...
class values_impl : public value_list {
 public:
  ...
  size_t size() const override { return num_values; }
...
base::metrics::impl::metric_id to_metrics_id(const type_instance_id &id);
/*!
 * \deprecated metrics registration should be done using the metrics layer
 *
 */
...
[[deprecated("Use the metrics layer")]] static type_instance_id add_polled_metric(const type_instance_id &id,
                                                                                  description d, Arg &&arg,
                                                                                  bool enabled = true) {
  namespace sm = base::metrics::impl;

  base::metrics::impl::get_local_impl()->add_registration(to_metrics_id(id), arg.type,
                                                          sm::make_function(arg.value, arg.type), d, enabled);
  return id;
}
/*!
 * \deprecated metrics registration should be done using the metrics layer
 ...
 */
template <typename Args>
[[deprecated("Use the metrics layer")]] static type_instance_id add_disabled_polled_metric(const type_instance_id &id,
                                                                                           description d, Args &&arg) {
...
#endif  // STONEDB_BASE_SCOLLECTD_H_
