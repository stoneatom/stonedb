...
#ifndef STONEDB_BASE_SEMAPHORE_H_
#define STONEDB_BASE_SEMAPHORE_H_
#pragma once
...
class broken_semaphore : public std::exception {
 public:
  ...
  const char *what() const noexcept override { return "Semaphore broken"; }
};
...
class semaphore_timed_out : public std::exception {
 public:
  /// Reports the exception reason.
  const char *what() const noexcept override { return "Semaphore timedout"; }
};
...
struct semaphore_default_exception_factory {
  static semaphore_timed_out timeout() { return semaphore_timed_out(); }
  static broken_semaphore broken() { return broken_semaphore(); }
};
...
///
/// \tparam ExceptionFactory template parameter allows modifying a semaphore to
/// throw customized exceptions on timeout/broken(). It has to provide two
/// static functions ExceptionFactory::timeout() and ExceptionFactory::broken()
/// which return corresponding exception object.
...
class basic_semaphore {
 ...
 private:
  ...
  struct expiry_handler {
    void operator()(entry &e) noexcept { e.pr.set_exception(ExceptionFactory::timeout()); }
  };
  expiring_fifo<entry, expiry_handler, clock> _wait_list;
  bool has_available_units(size_t nr) const { return _count >= 0 && (static_cast<size_t>(_count) >= nr); }
  bool may_proceed(size_t nr) const { return has_available_units(nr) && _wait_list.empty(); }

 public:
  /// Returns the maximum number of units the semaphore counter can hold
  static constexpr size_t max_counter() { return std::numeric_limits<decltype(_count)>::max(); }
  ...
  future<> wait(size_t nr = 1) { return wait(time_point::max(), nr); }
  /// Waits until at least a specific number of units are available in the
  ...
  ///         \ref semaphore_timed_out exception.  If the semaphore was
  ///         \ref broken(), may contain an exception.
  future<> wait(duration timeout, size_t nr = 1) { return wait(clock::now() + timeout, nr); }
  ...
  //
  /// Consume the specific number of units now, regardless of how many units are
  /// available in the counter, and reduces the counter by that amount of units.
  /// This operation may cause the counter to go negative.
  ///
  ...
  void ensure_space_for_waiters(size_t n) { _wait_list.reserve(n); }
};

template <typename ExceptionFactory, typename Clock>
inline void basic_semaphore<ExceptionFactory, Clock>::broken(std::exception_ptr xp) {
  _ex = xp;
  _count = 0;
...
class semaphore_units {
 ...
 public:
  ...
  semaphore_units(semaphore_units &&o) noexcept : _sem(o._sem), _n(o._n) { o._n = 0; }
  ...
  size_t release() { return std::exchange(_n, 0); }
};

/// \brief Take units from semaphore temporarily
///
/// Takes units from the semaphore and returns them when the \ref
/// semaphore_units object goes out of scope. This provides a safe way to
/// temporarily take units from a semaphore and ensure that they are eventually
/// returned under all circumstances (exceptions, premature scope exits, etc).
///
/// Unlike with_semaphore(), the scope of unit holding is not limited to the
/// scope of a single async lambda.
///
/// \param sem The semaphore to take units from
/// \param units  Number of units to take
/// \return a \ref future<> holding \ref semaphore_units object. When the object
/// goes out of scope
...
/// timeout is reached before units are granted throws semaphore_timed_out
/// exception.
///
/// \param sem The semaphore to take units from
/// \param units  Number of units to take
...
/// \related semaphore
template <typename ExceptionFactory, typename Clock = typename timer<>::clock>
future<semaphore_units<ExceptionFactory, Clock>> get_units(
    basic_semaphore<ExceptionFactory, Clock> &sem, size_t units,
    typename basic_semaphore<ExceptionFactory, Clock>::time_point timeout) {
  return sem.wait(timeout, units).then([&sem, units] { return semaphore_units<ExceptionFactory, Clock>{sem, units}; });
}

/// \brief Take units from semaphore temporarily with time bound on wait
///
/// Like \ref get_units(basic_semaphore<ExceptionFactory>&, size_t,
/// basic_semaphore<ExceptionFactory>::time_point) but allow the timeout to be
...
future<semaphore_units<ExceptionFactory>> get_units(basic_semaphore<ExceptionFactory> &sem, size_t units,
                                                    typename basic_semaphore<ExceptionFactory>::duration timeout) {
  return sem.wait(timeout, units).then([&sem, units] { return semaphore_units<ExceptionFactory>{sem, units}; });
}

/// \brief Consume units from semaphore temporarily
///
/// Consume units from the semaphore and returns them when the \ref
/// semaphore_units object goes out of scope. This provides a safe way to
/// temporarily take units from a semaphore and ensure that they are eventually
/// returned under all circumstances (exceptions, premature scope exits, etc).
///
...
inline futurize_t<std::result_of_t<Func()>> with_semaphore(basic_semaphore<ExceptionFactory, Clock> &sem, size_t units,
                                                           Func &&func) {
...
inline futurize_t<std::result_of_t<Func()>> with_semaphore(basic_semaphore<ExceptionFactory> &sem, size_t units,
                                                           typename basic_semaphore<ExceptionFactory>::duration timeout,
                                                           Func &&func) {
...
/// default basic_semaphore specialization that throws semaphore specific
...
#endif  // STONEDB_BASE_SEMAPHORE_H_
