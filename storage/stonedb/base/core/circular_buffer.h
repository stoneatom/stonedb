 ...
 */
#ifndef STONEDB_BASE_CIR_BUFF_H_
#define STONEDB_BASE_CIR_BUFF_H_
#pragma once
...
class circular_buffer {
 ...
 private:
  ...
  struct cbiterator : std::iterator<std::random_access_iterator_tag, ValueType> {
    ...
    bool operator>=(const cbiterator<CB, ValueType> &rhs) const { return idx >= rhs.idx; }
    bool operator<=(const cbiterator<CB, ValueType> &rhs) const { return idx <= rhs.idx; }
    typename super_t::difference_type operator-(const cbiterator<CB, ValueType> &rhs) const { return idx - rhs.idx; }
...
template <typename T, typename Alloc>
inline circular_buffer<T, Alloc>::circular_buffer(circular_buffer &&x) : _impl(std::move(x._impl)) {
  x._impl = {};
}
...
template <typename T, typename Alloc>
inline typename circular_buffer<T, Alloc>::iterator circular_buffer<T, Alloc>::erase(iterator first, iterator last) {
...
#endif  // STONEDB_BASE_CIR_BUFF_H_
