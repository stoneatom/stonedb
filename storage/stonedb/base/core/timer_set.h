...
#ifndef STONEDB_BASE_TIMER_SET_H_
#define STONEDB_BASE_TIMER_SET_H_
#pragma once
...
class timer_set {
 public:
  ...
  using timer_list_t =
      boost::intrusive::list<Timer, boost::intrusive::member_hook<Timer, boost::intrusive::list_member_hook<>, link>>;
 ...
 private:
  static timestamp_t get_timestamp(time_point _time_point) { return _time_point.time_since_epoch().count(); }

  static timestamp_t get_timestamp(Timer &timer) { return get_timestamp(timer.get_timeout()); }
  ...
  int get_index(Timer &timer) const { return get_index(get_timestamp(timer)); }

  int get_last_non_empty_bucket() const { return bitsets::get_last_set(_non_empty_buckets); }
 ...
 public:
   ...
   *
   * The value returned by timer.get_timeout() is used as timer's expiry. The
   * result of timer.get_timeout() must not change while the timer is in the
   * active set.
   *
   * Preconditions:
   *  - this timer must not be currently in the active set or in the expired
   * set.
   *
   * Postconditions:
   ...
   *
   * Returns true if and only if this timer's timeout is less than
   * get_next_timeout(). When this function returns true the caller should
   * reschedule expire() to be called at timer.get_timeout() to ensure timers
   * are expired in a timely manner.
   */
   ...
   *
   * Postconditons:
   *  - all timers from the active set with Timer::get_timeout() <= now are
   * moved to the expired set.
   */
   ...
   */
  time_point get_next_timeout() const { return time_point(duration(std::max(_last, _next))); }
   ...
   */
  bool empty() const { return _non_empty_buckets.none(); }

  time_point now() { return Timer::clock::now(); }
};

}  // namespace base
}  // namespace stonedb

#endif  // STONEDB_BASE_TIMER_SET_H_
