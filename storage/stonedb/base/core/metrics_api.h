 ...
 */
#ifndef STONEDB_BASE_METRICS_API_H_
#define STONEDB_BASE_METRICS_API_H_
#pragma once
...
namespace base {
namespace metrics {  // internal namespace
namespace impl {
...
namespace std {
template <>
struct hash<stonedb::base::metrics::impl::labels_type> {
  using argument_type = stonedb::base::metrics::impl::labels_type;
  using result_type = ::std::size_t;
  result_type operator()(argument_type const &s) const {
    result_type h = 0;
    for (auto &&i : s) {
      boost::hash_combine(h, std::hash<stonedb::base::sstring>{}(i.second));
    }
    return h;
  }
};
 ...
 *
 * Inherit type allows customizing one of the basic types (gauge, counter,
 * derive).
 *
...
class metric_id {
 public:
  ...
  const group_name_type &group_name() const { return _group; }
  void group_name(const group_name_type &name) { _group = name; }
  const instance_id_type &instance_id() const { return _labels.at(shard_label.name()); }
  const metric_name_type &name() const { return _name; }
  const metrics::metric_type_def &inherit_type() const { return _labels.at(type_label.name()); }
 ...
 private:
  auto as_tuple() const { return std::tie(group_name(), instance_id(), name(), inherit_type(), labels()); }
...
namespace std {
template <>
struct hash<stonedb::base::metrics::impl::metric_id> {
  using argument_type = stonedb::base::metrics::impl::metric_id;
  using result_type = ::std::size_t;
  result_type operator()(argument_type const &s) const {
    result_type const h1(std::hash<stonedb::base::sstring>{}(s.group_name()));
    result_type const h2(std::hash<stonedb::base::sstring>{}(s.instance_id()));
...
class metric_groups_impl : public metric_groups_def {
 ...
 public:
  ...
  metric_groups_impl &add_metric(group_name_type name, const metric_definition &md) override;
  metric_groups_impl &add_group(group_name_type name, const std::initializer_list<metric_definition> &l) override;
  metric_groups_impl &add_group(group_name_type name, const std::vector<metric_definition> &l) override;
};
...
class registered_metric {
 ...
 public:
  ...
  virtual metric_value operator()() const { return _f(); }

  bool is_enabled() const { return _info.enabled; }

  void set_enabled(bool b) { _info.enabled = b; }

  const metric_id &get_id() const { return _info.id; }

  const metric_info &info() const { return _info; }
  metric_function &get_function() { return _f; }
};
...
class metric_family {
 ...
 public:
  ...
  register_ref &operator[](const labels_type &l) { return _instances[l]; }

  const register_ref &at(const labels_type &l) const { return _instances.at(l); }

  metric_family_info &info() { return _info; }

  const metric_family_info &info() const { return _info; }

  iterator find(const labels_type &l) { return _instances.find(l); }

  const_iterator find(const labels_type &l) const { return _instances.find(l); }

  iterator begin() { return _instances.begin(); }

  const_iterator begin() const { return _instances.cbegin(); }

  iterator end() { return _instances.end(); }

  bool empty() const { return _instances.empty(); }

  iterator erase(const_iterator position) { return _instances.erase(position); }

  const_iterator end() const { return _instances.cend(); }

  uint32_t size() const { return _instances.size(); }
};
...
class impl {
 ...
 public:
  value_map &get_value_map() { return _value_map; }

  const value_map &get_value_map() const { return _value_map; }
  ...
  void set_config(const config &c) { _config = c; }
...
struct metric_settings {
  // Set the hostname used by the metrics, if not set, the local hostname will
  // be used.
  std::string hostname;
};

metric_settings create_default_metric_settings();

/*!
 * \brief set the metrics configuration
 */
future<> configure(const metric_settings &opts = create_default_metric_settings());
}  // namespace metrics

}  // namespace base
}  // namespace stonedb

#endif  // STONEDB_BASE_METRICS_API_H_
