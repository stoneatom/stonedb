 ...
 */
#ifndef STONEDB_BASE_CHECKED_PTR_H_
#define STONEDB_BASE_CHECKED_PTR_H_
#pragma once
...
/// \brief
/// Default not engaged base::checked_ptr dereferencing action (functor).
///
/// Throws a base::checked_ptr_is_null_exception.
///
struct default_null_deref_action {
  /// \throw base::checked_ptr_is_null_exception
  void operator()() const { throw checked_ptr_is_null_exception(); }
};

/// \cond internal
/// \namespace stonedb::base::internal
namespace internal {
/// \name base::checked_ptr::get() helpers
/// Helper functions that simplify the base::checked_ptr::get() implementation.
/// @{
...
template <typename T>
/// cond GCC6_CONCEPT_DOC - nested '\ cond' doesn't seem to work (bug 736553),
/// so working it around
GCC6_CONCEPT(requires requires(T ptr) { ptr.get(); })
    /// endcond
...
/// \class base::checked_ptr
/// \brief
/// base::checked_ptr class is a wrapper class that may be used with any pointer
/// type (smart like std::unique_ptr or raw pointers like int*).
///
/// The base::checked_ptr object will invoke the NullDerefAction functor if
/// it is dereferenced when the underlying pointer is not engaged.
///
/// It may still be assigned, compared to other base::checked_ptr objects or
/// moved without limitations.
///
/// The default NullDerefAction will throw a  base::default_null_deref_action
/// exception.
///
/// \tparam NullDerefAction a functor that is invoked when a user tries to
/// dereference a not engaged pointer.
///
...
GCC6_CONCEPT(requires std::is_default_constructible<NullDerefAction>::value &&requires(NullDerefAction action) {
 ...
 private:
  /// Invokes a NullDerefAction functor if the underlying pointer is not
  /// engaged.
  void check() const {
    if (!_ptr) {
      NullDerefAction()();
 ...
 public:
  checked_ptr() = default;
  checked_ptr(std::nullptr_t) noexcept(std::is_nothrow_default_constructible<checked_ptr<Ptr, NullDerefAction>>::value)
      : checked_ptr() {}
  ...
  /// \name Checked Methods
  /// These methods start with invoking a NullDerefAction functor if the
  /// underlying pointer is not engaged.
  /// @{

  /// Invokes the get() method of the underlying smart pointer or returns the
  /// pointer itself for a raw pointer (const variant). \return The pointer to
  ...
  size_t hash() const { return std::hash<Ptr>()(_ptr); }
  ///@}
};
...
namespace std {
/// std::hash specialization for base::checked_ptr class
template <typename T>
struct hash<base::checked_ptr<T>> {
  /// Get the hash value for the given base::checked_ptr object.
  /// The hash will calculated using the base::checked_ptr::hash method.
  /// \param p object for hash value calculation
  /// \return The hash value for the given object
  size_t operator()(const base::checked_ptr<T> &p) const { return p.hash(); }
};

}  // namespace std

#endif STONEDB_BASE_CHECKED_PTR_H_
