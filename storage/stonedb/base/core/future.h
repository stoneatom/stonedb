 ...
 */
#ifndef STONEDB_BASE_FUTURE_H_
#define STONEDB_BASE_FUTURE_H_
#pragma once
...
class future {
 ...
 private:
  ...
  void schedule(Func &&func) {
    if (state()->available()) {
      stonedb::base::schedule(std::make_unique<continuation<Func, T...>>(std::move(func), std::move(*state())));
    } else {
      assert(_promise);
 ...
 public:
  ...
  GCC6_CONCEPT(requires stonedb::base::CanApply<Func, T...>)
  ...
  GCC6_CONCEPT(requires stonedb::base::CanApply<Func, future>)
   ...
   */
  template <typename Func>
  GCC6_CONCEPT(requires stonedb::base::CanApply<Func>)
  ...
  template <typename Func>
  /* Broken?
  GCC6_CONCEPT( requires stonedb::base::ApplyReturns<Func, future<T...>,
  std::exception_ptr>
                  || (sizeof...(T) == 0 && stonedb::base::ApplyReturns<Func,
  void, std::exception_ptr>)
                  || (sizeof...(T) == 1 && stonedb::base::ApplyReturns<Func,
  T..., std::exception_ptr>)
  ) */
...
inline void promise<T...>::make_ready() noexcept {
  if (_task) {
    ...
    if (Urgent == urgent::yes && !need_preempt()) {
      stonedb::base::schedule_urgent(std::move(_task));
    } else {
      stonedb::base::schedule(std::move(_task));
    }
  }
}
...
typename futurize<T>::type futurize<T>::apply(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
  try {
    return convert(stonedb::base::apply(std::forward<Func>(func), std::move(args)));
  } catch (...) {
...
do_void_futurize_apply_tuple(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
  try {
    stonedb::base::apply(std::forward<Func>(func), std::move(args));
    return make_ready_future<>();
  } catch (...) {
    return make_exception_future(std::current_exception());
  }
...
do_void_futurize_apply_tuple(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
  try {
    return stonedb::base::apply(std::forward<Func>(func), std::move(args));
  } catch (...) {
    return make_exception_future(std::current_exception());
  }
}
...
inline future<T> futurize<T>::make_exception_future(Arg &&arg) {
  return stonedb::base::make_exception_future<T>(std::forward<Arg>(arg));
}
...
inline future<> futurize<void>::make_exception_future(Arg &&arg) {
  return stonedb::base::make_exception_future<>(std::forward<Arg>(arg));
}
