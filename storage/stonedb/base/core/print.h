 ...
 */
#ifndef STONEDB_BASE_PRINT_H_
#define STONEDB_BASE_PRINT_H_
#pragma once
...
std::ostream &fprint(std::ostream &os, const char *fmt, A &&...a) {
  stonedb::fmt::fprintf(os, fmt, std::forward<A>(a)...);
  return os;
}

template <typename... A>
void print(const char *fmt, A &&...a) {
  stonedb::fmt::printf(fmt, std::forward<A>(a)...);
}
...
std::string sprint(const sstring &fmt, A &&...a) {
  std::ostringstream os;
  stonedb::fmt::fprintf(os, fmt.c_str(), std::forward<A>(a)...);
  return os.str();
}
