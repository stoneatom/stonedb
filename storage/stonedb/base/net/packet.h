 ...
 */
#ifndef STONEDB_BASE_PACKET_H_
#define STONEDB_BASE_PACKET_H_
#pragma once
...
class packet final {
  ...
  struct impl {
    ...
    void operator delete(void *ptr, [[maybe_unused]] size_t nr_frags) { return ::operator delete(ptr); }
 ...
 public:
  ...
  std::experimental::optional<uint32_t> rss_hash() { return _impl->_rss_hash; }
  std::experimental::optional<uint32_t> set_rss_hash(uint32_t hash) { return _impl->_rss_hash = hash; }
  ...
  explicit operator bool() { return bool(_impl); }
  static packet make_null_packet() { return net::packet(nullptr); }
...
inline packet::impl::impl(size_t nr_frags) : _len(0), _allocated_frags(nr_frags) {}
...
inline bool packet::allocate_headroom(size_t size) {
  if (_impl->_headroom >= size) {
    ...
    if (!_impl->using_internal_data()) {
      ...
      std::copy_backward(_impl->_frags, _impl->_frags + _impl->_nr_frags, _impl->_frags + _impl->_nr_frags + 1);
...
inline packet::packet(packet &&x, fragment frag, deleter d) : _impl(impl::allocate_if_needed(std::move(x._impl), 1)) {
  _impl->_len += frag.size;
  _impl->_frags[_impl->_nr_frags++] = frag;
...
inline packet::packet(packet &&x, deleter d) : _impl(std::move(x._impl)) { _impl->_deleter.append(std::move(d)); }

inline packet::packet(packet &&x, temporary_buffer<char> buf)
    : packet(std::move(x), fragment{buf.get_write(), buf.size()}, buf.release()) {}

inline packet::packet(temporary_buffer<char> buf) : packet(fragment{buf.get_write(), buf.size()}, buf.release()) {}
...
inline packet packet::share() { return share(0, _impl->_len); }
