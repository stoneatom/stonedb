 ...
 */
#ifndef TIANMU_BASE_POSIX_STACK_H_
#define TIANMU_BASE_POSIX_STACK_H_
#pragma once
...
// We can't keep this in any of the socket servers as instance members, because
// a connection can outlive the socket server. To avoid having the whole
// socket_server tracked as a shared pointer, we will have a conntrack
// structure.
//
// Right now this class is used by the posix_server_socket_impl, but it could be
// used by any other.
class conntrack {
  class load_balancer {
   ...
   public:
    ...
    void closed_cpu(shard_id cpu) { _cpu_load[cpu]--; }
    shard_id next_cpu() {
      // FIXME: The naive algorithm will just round robin the connections around
      // the shards. A more complex version can keep track of the amount of
      // activity in each connection, and use that information.
  ...
  void closed_cpu(shard_id cpu) { _lb->closed_cpu(cpu); }
  ...
  class handle {
   ...
   public:
    ...
    handle(shard_id cpu, lw_shared_ptr<load_balancer> lb)
        : _host_cpu(engine().cpu_id()), _target_cpu(cpu), _lb(make_foreign(std::move(lb))) {}
    ...
    ~handle() {
      ...
      smp::submit_to(_host_cpu, [cpu = _target_cpu, lb = std::move(_lb)] { lb->closed_cpu(cpu); });
    }
    shard_id cpu() { return _target_cpu; }
  };
  friend class handle;

  conntrack() : _lb(make_lw_shared<load_balancer>()) {}
  handle get_handle() { return handle(_lb->next_cpu(), _lb); }
};
...
class posix_network_stack : public network_stack {
 ...
 public:
  posix_network_stack() : _reuseport(engine().posix_reuseport_available()) {}
  virtual server_socket listen(socket_address sa, listen_options opts) override;
  virtual Tianmu::base::socket socket() override;
...
class posix_ap_network_stack : public posix_network_stack {
 ...
 public:
  posix_ap_network_stack() : posix_network_stack(), _reuseport(engine().posix_reuseport_available()) {}
  virtual server_socket listen(socket_address sa, listen_options opts) override;
  static future<std::unique_ptr<network_stack>> create() {
    return make_ready_future<std::unique_ptr<network_stack>>(
        std::unique_ptr<network_stack>(new posix_ap_network_stack()));
  }
};
...
#endif  // TIANMU_BASE_POSIX_STACK_H_
