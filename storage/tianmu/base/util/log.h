 ...
 */
#ifndef TIANMU_BASE_LOG_H_
#define TIANMU_BASE_LOG_H_
#pragma once
...
// Boost doesn't auto-deduce the existence of the streaming operators for some
// reason
...
/// \code {.cpp}
/// static base::logger logger("lsa-api");
...
class logger {
 ...
 public:
  ...
  bool is_enabled(log_level level) const { return level <= _level.load(std::memory_order_relaxed); }
  ...
  ///
  log_level level() const { return _level.load(std::memory_order_relaxed); }
