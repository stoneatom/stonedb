...
namespace boost {
template <>
Tianmu::base::log_level lexical_cast(const std::string &source) {
  std::istringstream in(source);
  Tianmu::base::log_level level;
  if (!(in >> level)) {
...
std::ostream &operator<<(std::ostream &out, const std::exception_ptr &eptr) {
  ...
  try {
    ...
    if (tp) {
      out << Tianmu::base::pretty_type_name(*tp);
    } else {
...
std::ostream &operator<<(std::ostream &out, const std::exception &e) {
  return out << Tianmu::base::pretty_type_name(typeid(e)) << " (" << e.what() << ")";
}

std::ostream &operator<<(std::ostream &out, const std::system_error &e) {
  return out << Tianmu::base::pretty_type_name(typeid(e)) << " (error " << e.code() << ", " << e.code().message()
             << ")";
}
