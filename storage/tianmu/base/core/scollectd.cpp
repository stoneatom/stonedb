...
namespace scollectd {
base::logger logger("scollectd");
thread_local unsigned type_instance_id::_next_truncated_idx = 0;

registration::~registration() { unregister(); }

registration::registration(const type_instance_id &id) : _id(id), _impl(base::metrics::impl::get_local_impl()) {}

registration::registration(type_instance_id &&id) : _id(std::move(id)), _impl(base::metrics::impl::get_local_impl()) {}

base::metrics::impl::metric_id to_metrics_id(const type_instance_id &id) {
  return base::metrics::impl::metric_id(id.plugin(), id.type_instance(),
                                        {{base::metrics::shard_label.name(), base::metrics::impl::shard()},
                                         {base::metrics::type_label.name(), id.type()}});
}
...
enum class part_type : uint16_t {
  ...
  Time = 0x0001,        // Time  Numeric The timestamp to associate with subsequent
                        // data values, unix time format (seconds since epoch)
  TimeHr = 0x0008,      // Time (high resolution)  Numeric The timestamp to
                        // associate with subsequent data values. Time is defined in
                        // 2–30 seconds since epoch. New in Version 5.0.
  Plugin = 0x0002,      // Plugin String The plugin name to associate with
                        // subsequent data values, e.g. "cpu"
  PluginInst = 0x0003,  // Plugin instance String  The plugin instance name to
                        // associate with subsequent data values, e.g. "1"
  Type = 0x0004,        // Type String The type name to associate with subsequent data
                        // values, e.g. "cpu"
  TypeInst = 0x0005,    // Type instance     String  The type instance name to
                        // associate with subsequent data values, e.g. "idle"
  Values = 0x0006,      // Values  other   Data values, see above
  Interval = 0x0007,    // Interval Numeric Interval used to set the "step" when creating
                        // new RRDs unless rrdtool plugin forces StepSize. Also used to
                        // detect values that have timed out.
  IntervalHr = 0x0009,  // Interval (high resolution)  Numeric     The interval in which
                        // subsequent data values are collected. The interval is given in
                        // 2–30 seconds. New in Version 5.0.
...
using collectd_hres_duration = std::chrono::duration<uint64_t, std::ratio<1, 0x40000000>>;

// yet another writer type, this one to construct collectd network
// protocol data.
struct cpwriter {
  using buffer_type = std::array<char, payload_size>;
  using mark_type = buffer_type::iterator;
  using const_mark_type = buffer_type::const_iterator;
  ...
  mark_type mark() const { return _pos; }
  bool overflow() const { return _overflow; }
  void reset(mark_type m) {
    _pos = m;
    _overflow = false;
  }
  size_t size() const { return std::distance(_buf.begin(), const_mark_type(_pos)); }
  bool empty() const { return _pos == _buf.begin(); }
  void clear() {
    reset(_buf.begin());
    _cache.clear();
    _overflow = false;
  }
  const char *data() const { return &_buf.at(0); }
  char *data() { return &_buf.at(0); }
  ...
  sstring get_type_instance(const base::metrics::impl::metric_id &id) {
    if (id.labels().empty()) {
      return id.name();
    }
    sstring res = id.name();
    for (auto i : id.labels()) {
      if (i.first != base::metrics::shard_label.name() && i.first != base::metrics::type_label.name()) {
        res += "-" + i.second;
      }
    }
    return res;
  }
  explicit operator bool() const { return !_overflow; }
  ...
  void write_value(const base::metrics::impl::metric_value &v) {
    switch (v.type()) {
  ...
  cpwriter &put(part_type type, const base::metrics::impl::metric_value &v) {
  ...
  cpwriter &put(const sstring &host, const base::metrics::impl::metric_id &id) {
    ...
    put_cached(part_type::PluginInst,
               id.instance_id() == per_cpu_plugin_instance ? to_sstring(engine().cpu_id()) : id.instance_id());
    put_cached(part_type::Type, id.inherit_type());
    // Optional
    put_cached(part_type::TypeInst, get_type_instance(id));
    return *this;
  }
  cpwriter &put(const sstring &host, const duration &period, const type_instance_id &id, const value_list &v) {
  ...
  cpwriter &put(const sstring &host, const duration &period, const base::metrics::impl::metric_id &id,
                const base::metrics::impl::metric_value &v) {
    const auto ps = std::chrono::duration_cast<collectd_hres_duration>(period).count();
    put(host, id);
    put(part_type::Values, v);
    if (ps != 0) {
...
void impl::add_polled([[maybe_unused]] const type_instance_id &id,
                      [[maybe_unused]] const shared_ptr<value_list> &values, [[maybe_unused]] bool enable) {
  // do nothing
  // add_polled is now implemented on the metrics layer
}

void impl::remove_polled(const type_instance_id &id) { base::metrics::impl::unregister_metric(to_metrics_id(id)); }
...
void impl::start(const sstring &host, const ipv4_addr &addr, const duration period) {
  ...
  send_notification(type_instance_id("scollectd", per_cpu_plugin_instance, "network"), "daemon started");
  arm();
}
...
void impl::run() {
  using metric_family_id = size_t;
  using value_iterator = base::metrics::impl::value_vector::iterator;
  using metadata_iterator = base::metrics::impl::metric_metadata_vector::iterator;
  using context = std::tuple<metric_family_id, metadata_iterator, value_iterator, cpwriter>;

  auto ctxt = make_lw_shared<context>();
  foreign_ptr<shared_ptr<base::metrics::impl::values_copy>> vals = base::metrics::impl::get_values();
  ...
  auto send_packet = [this, ctxt, &values, metadata]() mutable {
    ...
    while (!out_of_space && mf < values.size()) {
      while (i != values[mf].end()) {
        if (i->type() == base::metrics::impl::data_type::HISTOGRAM) {
          ++i;
          ++md_iterator;
          continue;
        }
  ...
  do_until(stop_when, send_packet).finally([this, vals = std::move(vals)]() mutable { arm(); });
}
...
void remove_polled_metric(const type_instance_id &id) { get_impl().remove_polled(id); }
...
void configure(const collectd_options &options) {
  if (!options.enabled) {
    return;
  }
  auto addr = ipv4_addr(options.address);
  auto period = std::chrono::milliseconds(options.poll_period);

  auto host = base::metrics::impl::get_local_impl()->get_config().hostname;

  // Now create send loops on each cpu
  for (unsigned c = 0; c < smp::count; c++) {
    smp::submit_to(c, [=]() { get_impl().start(host, addr, period); });
  }
}

static base::metrics::impl::register_ref get_register(const scollectd::type_instance_id &i) {
  base::metrics::impl::metric_id id = to_metrics_id(i);
  return base::metrics::impl::get_value_map().at(id.full_name()).at(id.labels());
}

std::vector<collectd_value> get_collectd_value(const scollectd::type_instance_id &id) {
  std::vector<collectd_value> vals;
  const base::metrics::impl::registered_metric &val = *get_register(id);
  vals.push_back(val());
  return vals;
}

std::vector<scollectd::type_instance_id> get_collectd_ids() { return get_impl().get_instance_ids(); }

bool is_enabled(const scollectd::type_instance_id &id) { return get_register(id)->is_enabled(); }
...
void plugin_instance_metrics::add(const typed_value &v) { _registrations.emplace_back(add_impl(v)); }
