...
#ifndef TIANMU_BASE_POSIX_H_
#define TIANMU_BASE_POSIX_H_
#pragma once
...
class file_desc {
 ...
 public:
  ...
  int ioctl(int request) { return ioctl(request, 0); }
  ...
  mmap_area map_shared_rw(size_t size, size_t offset) { return map(size, PROT_READ | PROT_WRITE, MAP_SHARED, offset); }

  mmap_area map_shared_ro(size_t size, size_t offset) { return map(size, PROT_READ, MAP_SHARED, offset); }

  mmap_area map_private_rw(size_t size, size_t offset) {
    return map(size, PROT_READ | PROT_WRITE, MAP_PRIVATE, offset);
  }

  mmap_area map_private_ro(size_t size, size_t offset) { return map(size, PROT_READ, MAP_PRIVATE, offset); }
...
inline void pin_this_thread(unsigned cpu_id) {
  ...
  CPU_ZERO(&cs);
  CPU_SET(cpu_id, &cs);
  auto r [[maybe_unused]] = pthread_setaffinity_np(pthread_self(), sizeof(cs), &cs);
  assert(r == 0);
}
