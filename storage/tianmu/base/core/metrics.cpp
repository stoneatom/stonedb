...
void metric_groups::clear() { _impl = impl::create_metric_groups(); }
...
metric_group_definition::metric_group_definition(const group_name_type &name,
                                                 std::initializer_list<metric_definition> l)
    : name(name), metrics(l) {}
...
metric_settings create_default_metric_settings() { return metric_settings{get_hostname()}; }

future<> configure(const metric_settings &opts) {
  impl::config c;
...
namespace impl {
registered_metric::registered_metric(metric_id id, metric_function f, bool enabled) : _f(f), _impl(get_local_impl()) {
...
metric_definition_impl::metric_definition_impl(metric_name_type name, metric_type type, metric_function f,
                                               description d, std::vector<label_instance> _labels)
    : name(name), type(type), f(f), d(d), enabled(true) {
  for (auto i : _labels) {
...
std::unique_ptr<metric_groups_def> create_metric_groups() { return std::make_unique<metric_groups_impl>(); }
...
bool metric_id::operator<(const metric_id &id2) const { return as_tuple() < id2.as_tuple(); }
...
sstring metric_id::full_name() const { return safe_name(_group + "_" + _name); }
...
shared_ptr<impl> get_local_impl() {
  static thread_local auto the_impl = base::make_shared<impl>();
  return the_impl;
}
...
void unregister_metric(const metric_id &id) { get_local_impl()->remove_registration(id); }

const value_map &get_value_map() { return get_local_impl()->get_value_map(); }

foreign_ptr<values_reference> get_values() {
  shared_ptr<values_copy> res_ref = base::make_shared<values_copy>();
...
void impl::update_metrics_if_needed() {
  if (_dirty) {
    ...
    _metadata = base::make_shared<metric_metadata>();

    auto mt_ref = base::make_shared<metric_metadata>();
    auto &mt = *(mt_ref.get());
...
void impl::add_registration(const metric_id &id, data_type type, metric_function f, const description &d,
                            bool enabled) {
  auto rm = base::make_shared<registered_metric>(id, f, enabled);
