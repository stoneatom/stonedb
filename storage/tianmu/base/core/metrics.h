 ...
 */
#ifndef TIANMU_BASE_METRICS_H_
#define TIANMU_BASE_METRICS_H_
#pragma once
 ...
 *
 *  This header file contains the metrics creation method with their helper
 * function. Include this file when need to create metrics. Typically this will
...
namespace base {
/*!
 * \namespace Tianmu::base::metrics
 * \brief metrics creation and registration
 *
 * the metrics namespace holds the relevant method and classes to generate
 * metrics.
 *
 * The metrics layer support registering metrics, that later will be
 * exported via different API protocols.
 *
 * To be able to support multiple protocols the following simplifications where
 * made:
 ...
 *   _metrics->add_group("cache", {sm::make_gauge("bytes", "used", [this] {
 * return _region.occupancy().used_space();
 * })});
 * }
 * \endcode
 */
 ...
 *   sm::make_gauge("metric_name", value, description("A documentation about the
 * return value"))
 * });
 * \endcode
 *
...
class description {
 public:
  ...
  const sstring &str() const { return _s; }
...
class label_instance {
 ...
 public:
   ...
   */
  const sstring key() const { return _key; }
 ...
 * label is a label factory, you create it once, and use it to create the
 * label_instance.
 *
 * In the example we would like to label the smp_queue with with the queue owner
 *
 * base::metrics::label smp_owner("smp_owner");
 *
...
class label {
 ...
 public:
   ...
   * (ie. if it support the redirection operator for stringstream).
   *
   * All primitive types are supported so if lab is a label, all the following
   * examples are valid: lab(1) lab("my_value") lab(-1)
   */
  template <typename T>
   ...
   */
  const sstring &name() const { return key; }
};

/*!
 * \namesapce impl
 * \brief holds the implementation parts of the metrics layer, do not use
 * directly.
 *
 * The metrics layer define a thin API for adding metrics.
 * Some of the implementation details need to be in the header file, they should
 * not be use directly.
 */
namespace impl {
...
struct metric_value {
  ...
  double d() const { return boost::get<double>(u); }

  uint64_t ui() const { return boost::get<double>(u); }

  int64_t i() const { return boost::get<double>(u); }
  ...
  metric_value operator+(const metric_value &c);
  const histogram &get_histogram() const { return boost::get<histogram>(u); }
};
...
struct metric_definition_impl {
  ...
  metric_definition_impl(metric_name_type name, metric_type type, metric_function f, description d,
...
template <typename T>
struct is_callable<
    T, typename std::integral_constant<bool, !std::is_void<typename std::result_of<T()>::type>::value>::type>
...
/*
 * The metrics definition are defined to be compatible with collectd metrics
 * defintion. Typically you should used gauge or derived.
 */
...
impl::metric_definition_impl make_gauge(metric_name_type name, T &&val, description d = description(),
                                        std::vector<label_instance> labels = {}) {
  return {
      name, {impl::data_type::GAUGE, "gauge"}, make_function(std::forward<T>(val), impl::data_type::GAUGE), d, labels};
}
 ...
 */
template <typename T>
impl::metric_definition_impl make_gauge(metric_name_type name, description d, std::vector<label_instance> labels,
                                        T &&val) {
  return {
      name, {impl::data_type::GAUGE, "gauge"}, make_function(std::forward<T>(val), impl::data_type::GAUGE), d, labels};
}
 ...
 *
 * Derive is an integer value that can increase or decrease, typically it is
 * used when looking at the derivation of the value.
 *
 * It is OK to use it when counting things and if no wrap-around is expected (it
 * shouldn't) it's prefer over counter metric.
 */
template <typename T>
...
impl::metric_definition_impl make_derive(metric_name_type name, description d, T &&val) {
  return {
      name, {impl::data_type::DERIVE, "derive"}, make_function(std::forward<T>(val), impl::data_type::DERIVE), d, {}};
}
 ...
 *
 * Counters are similar to derived, but they assume monotony, so if a counter
 * value decrease in a series it is count as a wrap-around. It is better to use
 * large enough data value than to use counter.
 *
 */
template <typename T>
 ...
 *
 * Absolute are used for metric that are being erased after each time they are
 * read. They are here for compatibility reasons and should general be avoided
 ...
 */
template <typename T>
impl::metric_definition_impl make_histogram(metric_name_type name, description d, std::vector<label_instance> labels,
                                            T &&val) {
  return {name,
          {impl::data_type::HISTOGRAM, "histogram"},
          make_function(std::forward<T>(val), impl::data_type::HISTOGRAM),
...
impl::metric_definition_impl make_queue_length(metric_name_type name, T &&val, description d = description(),
                                               std::vector<label_instance> labels = {},
                                               [[maybe_unused]] instance_id_type instance = impl::shard()) {
...
template <typename T>
impl::metric_definition_impl make_total_operations(metric_name_type name, T &&val, description d = description(),
                                                   std::vector<label_instance> labels = {},
                                                   instance_id_type instance = impl::shard()) {
