...
class file_data_source_impl : public data_source_impl {
 ...
 private:
  ...
  unsigned get_initial_read_ahead() const {
    return _options.dynamic_adjustments ? std::min(_options.dynamic_adjustments->read_ahead, _options.read_ahead)
                                        : !!_options.read_ahead;
  }
  ...
  void issue_read_aheads(unsigned additional = 0) {
    ...
    while (_read_buffers.size() < ra) {
      ...
      _read_buffers.emplace_back(
          _pos, actual_size,
          futurize<future<temporary_buffer<char>>>::apply([&] {
            return _file.dma_read_bulk<char>(start, len, _options.io_priority_class);
          }).then_wrapped([this, start, pos = _pos, remain = _remain](future<temporary_buffer<char>> ret) {
            --_reads_in_progress;
...
input_stream<char> make_file_input_stream(file f, uint64_t offset, uint64_t len, file_input_stream_options options) {
...
class file_data_sink_impl : public data_sink_impl {
 ...
 public:
  ...
  future<> put([[maybe_unused]] net::packet data) override { abort(); }
  ...
  future<> do_put(uint64_t pos, temporary_buffer<char> buf) noexcept {
    try {
      ...
      return _file.dma_write(pos, p, buf_size, _options.io_priority_class)
          .then([this, buf = std::move(buf), truncate]([[maybe_unused]] size_t size) {
            if (truncate) {
...
class file_data_sink : public data_sink {
 public:
  file_data_sink(file f, file_output_stream_options options)
      : data_sink(std::make_unique<file_data_sink_impl>(std::move(f), options)) {}
};
