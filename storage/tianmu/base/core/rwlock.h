 ...
 */
#ifndef TIANMU_BASE_RWLOCK_H_
#define TIANMU_BASE_RWLOCK_H_
#pragma once
...
/// Acquiring the write lock will effectively cause all readers not to be
/// executed until the write part is done.
class rwlock : private rwlock_for_read, rwlock_for_write {
 ...
 public:
  ...
  /// Cast this rwlock into read lock object with lock semantics appropriate to
  /// be used by "with_lock". The resulting object will have lock / unlock calls
  /// that, when called, will acquire / release the lock in read mode.
  rwlock_for_read &for_read() { return *this; }

  /// Cast this rwlock into write lock object with lock semantics appropriate to
  /// be used by "with_lock". The resulting object will have lock / unlock calls
  /// that, when called, will acquire / release the lock in write mode.
  ...
  future<> read_lock() { return _sem.wait(); }
  ...
  /// not to execute.
  future<> write_lock() { return _sem.wait(max_ops); }

  /// Releases the lock, which must have been taken in write mode. After this
  /// is called, one of the other fibers waiting on \ref write_lock or the
  /// fibers waiting on \ref read_lock will be allowed to proceed.
  void write_unlock() {
#ifdef SEASTAR_DEBUG_LOCKING
  ...
  /// Tries to acquire the lock in read mode iff this can be done without
  /// waiting.
  bool try_read_lock() { return _sem.try_wait(); }

  /// Tries to acquire the lock in write mode iff this can be done without
  /// waiting.
  bool try_write_lock() { return _sem.try_wait(max_ops); }
  friend class rwlock_for_read;
  friend class rwlock_for_write;
};

/// \cond internal
inline future<> rwlock_for_read::lock() { return static_cast<rwlock *>(this)->read_lock(); }

inline void rwlock_for_read::unlock() { static_cast<rwlock *>(this)->read_unlock(); }

inline future<> rwlock_for_write::lock() { return static_cast<rwlock *>(this)->write_lock(); }

inline void rwlock_for_write::unlock() { static_cast<rwlock *>(this)->write_unlock(); }
/// \endcond
