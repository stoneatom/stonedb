...
When releasing the old space, the transaction ID of each DPN will be determined. If it is smaller than the current
transaction ID, it will be skipped directly and will not be released. In other words, releasing the waste space will
only release the waste space generated by the current transaction. This logic can ensure that, The data of the old
transaction will not be changed, and if the current transaction fails, it can be rolled back to the last most recently
...
void ColumnShare::alloc_seg(DPN *dpn) {
  ...
  for (auto it = segs.cbegin(); it != segs.cend(); ++it) {
    // Use overwrite if there is usable waste space
