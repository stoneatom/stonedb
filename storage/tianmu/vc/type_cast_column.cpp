...
double String2NumCastColumn::GetValueDoubleImpl(const core::MIIterator &mit) {
  if (full_const_)
    ...
    if (rs.IsNull()) {
      ...
      types::RCNum::Parse(rs, rcn, common::ColumnType::FLOAT);
      // if(types::RCNum::Parse(rs, rcn, common::ColumnType::FLOAT) !=
    ...
    if (val_ != common::NULL_VALUE_64) {
      if (TypeName() == common::ColumnType::TIME) {
        ...
        if (ltime.neg == 1)
          ...
          timehour = ltime.hour;
        types::RCDateTime rctime(timehour, minute, second, common::ColumnType::TIME);
        rcdt = rctime;
      } else {
