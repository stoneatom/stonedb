...
LoadParser::LoadParser(RCAttrPtrVect_t &attrs, const system::IOParameters &iop, uint packsize,
  ...
  tab_index_ = ha_rcengine_->GetTableIndex("./" + ioparam_.TableName());
}
      ...
      case ParsingStrategy::ParseResult::OK:
        ...
        if (make_value_ok) {
          ...
          if (tab_index_ != nullptr) {
            if (HA_ERR_FOUND_DUPP_KEY == ProcessInsertIndex(tab_index_, value_buffers, row_no_ - 1)) {
              row_no_--;
              dup_no_++;
...
int LoadParser::ProcessInsertIndex(std::shared_ptr<index::RCTableIndex> tab, std::vector<ValueCache> &vcs,
  ...
  if (tab->InsertIndex(current_txn_, fields, no_obj_ + no_rows) == common::ErrorCode::DUPP_KEY) {
...
int LoadParser::binlog_loaded_block(const char *buf_start, const char *buf_end) {
  ...
  lf_info = static_cast<LOAD_FILE_INFO *>(ioparam_.GetLogInfo());
