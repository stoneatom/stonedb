...
int ReadBuffer::Read(char *buffer, int bytes_to_read) {
  ...
  while (no_attempts < 16 && bytes_read != requested_size) {
    ...
    while (!do_stop) {
      try {
        read_b = tianmu_stream_->Read(buffer + bytes_read, std::min(bytes_to_read, requested_size - bytes_read));
      } catch (common::DatabaseException &) {
        read_b = -1;
      }
      if (read_b == -1)
...
void ReadBuffer::ReadThread() {
  ...
  while (!do_stop) {
    ...
    {
      std::scoped_lock guard(read_mutex_);
      if (!buf_incomplete_ || !tianmu_stream_->IsOpen() || stop_reading_thread_) {
        break;
      }
      ...
      if (bytes_in_read_thread_buffer_ != size_) {
        ...
        while (no_steps < 15 && to_read > 0) {
          try {
            no_read_bytes = tianmu_stream_->Read(read_thread_buffer_.get() + bytes_in_read_thread_buffer_, to_read);
          } catch (common::DatabaseException &) {
            no_read_bytes = -1;
          }
