...
TCMHeap::TCMHeap(size_t heap_size) : HeapPolicy(heap_size) {
  if (heap_size > 0)
    heap_.GrowHeap((heap_size) >> kPageShift);
  size_map_.Init();
  for (size_t i = 0; i < kNumClasses; i++) freelist_[i].Init();
}
...
void *TCMHeap::alloc(size_t size) {
  ...
  if (size > kMaxSize) {
    ...
    s->size = uint(size);
    s->sizeclass = 0;
    heap_.RegisterSizeClass(s, 0);
    return reinterpret_cast<void *>(s->start << kPageShift);
  }
  const size_t cl = size_t(size_map_.SizeClass((int)size));
  const size_t alloc_size = size_map_.ByteSizeForClass(cl);
  FreeList *list = &freelist_[cl];
  if (list->empty()) {
    // Get a new span of pages (potentially large enough for multiple
    // allocations of this size)
    int pages = int(size_map_.class_to_pages(cl));
    tcm::Span *res = heap_.New(pages);
    if (res == nullptr)
      return nullptr;

    heap_.RegisterSizeClass(res, cl);
  ...
  res = list->Pop();
  tcm::Span *s = heap_.GetDescriptor(ADDR_TO_PAGEID(res));
  s->refcount++;
  return res;
}
...
size_t TCMHeap::getBlockSize(void *mh) {
  size_t result;
  tcm::Span *span = heap_.GetDescriptor(ADDR_TO_PAGEID(mh));
  ASSERT(span != nullptr);
  if (span->sizeclass == 0)
    result = span->size;
  else
    result = size_map_.ByteSizeForClass(span->sizeclass);
...
void *TCMHeap::rc_realloc(void *mh, size_t size) {
  ...
  if (span->sizeclass == 0) {
    ...
    std::memcpy(res, mh, std::min(size_map_.ByteSizeForClass(span->sizeclass), size));
    dealloc(mh);
  }
