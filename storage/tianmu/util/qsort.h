...
using comp_func_tianmu = int (*)(const void *, const void *);

inline void __tianmu_swap_local(char *a, char *b, int len) {
  for (int j = 0; j < len; j++) {
    char t = a[j];
    a[j] = b[j];
    b[j] = t;
...
static void __bubble_sort(char *b, int lo, int hi, int s, comp_func_tianmu cmpsrt) {
  bool swapped = true;
  int i;
  ...
  while (swapped) {
    ...
    for (i = lo; i < hi; i++) {
      if (cmpsrt((void *)(b + i * s), (void *)(b + (i + 1) * s)) > 0) {
        __tianmu_swap_local(b + i * s, b + (i + 1) * s, s);
        swapped = true;
      }
    }
...
static void __quicksort_tianmu(char *b, int lo, int hi, int s, comp_func_tianmu cmpsrt, int call_seq) {
  if (call_seq > 8192) {
  ...
  while (i <= j) {
    ...
    if (i <= j) {
      __tianmu_swap_local(b + ii, b + jj, s);
      i++;
      ii += s;
      j--;
      jj -= s;
