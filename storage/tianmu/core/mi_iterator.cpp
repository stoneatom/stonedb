...
MIIterator::MIIterator(const MIIterator &sec, bool lock)
  ...
  if (sec.mind_created_locally) {
    ...
    dg[0] = mind->dim_groups_[0];  // assuming that the local minds are
                                   // one-dimensional
  }
...
void MIIterator::Init(bool lock) {
  ...
  bool *dim_group_used = new bool[mind->dim_groups_.size()];

  for (uint i = 0; i < mind->dim_groups_.size(); i++) dim_group_used[i] = false;

  for (int i = 0; i < no_dims; i++)
    if (dimensions[i]) {
      dim_group_used[mind->group_num_for_dimension_[i]] = true;
      mind->LockForGetIndex(i);
    }
  ...
  for (uint i = 0; i < mind->dim_groups_.size(); i++) {
    if (dim_group_used[i]) {
      no_obj = SafeMultiplication(no_obj, mind->dim_groups_[i]->NumOfTuples());
      zero_tuples = false;
    } else {
      omitted_factor = SafeMultiplication(omitted_factor, mind->dim_groups_[i]->NumOfTuples());
    }
  }
  ...
  if (no_dims > 0) {
    ...
    if (po.size() > 0) {
      for (int i = 0; i < no_dims; i++)
        if (dim_group_used[mind->group_num_for_dimension_[i]] && mind->GetFilter(i) && po[i].Initialized()) {
          DEBUG_ASSERT(mind->IsOrderable(i));

          it.push_back(mind->dimension_group_[i]->NewOrderedIterator(dimensions, &po[i], p_power));
          dg.push_back(mind->dimension_group_[i]);
          dim_group_used[mind->group_num_for_dimension_[i]] = false;
        }
    }
    ...
    for (uint i = 0; i < mind->dim_groups_.size(); i++)
      if (dim_group_used[i] && (mind->dim_groups_[i]->Type() == DimensionGroup::DGType::DG_FILTER ||
                                mind->dim_groups_[i]->Type() == DimensionGroup::DGType::DG_VIRTUAL))  // filters first
        ordering_filters.push_back(std::pair<int, int>(65537 - mind->dim_groups_[i]->GetFilter(-1)->DensityWeight(),
                                                       i));  // -1: the default filter for this group

    // order filters starting from the // densest one (for pack decompression efficiency)
    ...
    for (uint i = 0; i < ordering_filters.size(); i++) {  // create iterators for DimensionGroup numbers from sorter
      it.push_back(mind->dim_groups_[ordering_filters[i].second]->NewIterator(dimensions, p_power));
      dg.push_back(mind->dim_groups_[ordering_filters[i].second]);
    ...
    for (uint i = 0; i < mind->dim_groups_.size(); i++)
      if (dim_group_used[i]) {
        it.push_back(mind->dim_groups_[i]->NewIterator(dimensions, p_power));
        dg.push_back(mind->dim_groups_[i]);
      }

    for (int i = 0; i < no_dims; i++)
      if (dimensions[i]) {
        for (uint j = 0; j < dg.size(); j++) {
          if (mind->dimension_group_[i] == dg[j]) {
            it_for_dim[i] = j;
            break;
          }
        }
