...
common::RoughSetValue RCAttr::RoughCheck(int pack, Descriptor &d, bool additional_nulls_possible) {
    ...
    if (d.op == common::Operator::O_IS_NULL || d.op == common::Operator::O_NOT_NULL) {
      ...
      if (vc1->IsConst()) {
        ...
        if (v1 > dpn.max_i || v2 < dpn.min_i) {
          ...
          res = (mvc->Contains(mit, *rcvo) != false) ? common::RoughSetValue::RS_ALL : common::RoughSetValue::RS_NONE;
        } else {
...
common::RoughSetValue RCAttr::RoughCheck(int pack1, int pack2, Descriptor &d) {
  vcolumn::VirtualColumn *vc1 = d.val1.vc;
  vcolumn::VirtualColumn *vc2 = d.val2.vc;
...
// AND max"
common::RoughSetValue RCAttr::RoughCheckBetween(int pack, int64_t v1, int64_t v2) {
...
int64_t RCAttr::RoughMin(Filter *f, common::RoughSetValue *rf)  // f == nullptr is treated as full filter
{
  LoadPackInfo();
...
int64_t RCAttr::RoughMax(Filter *f, common::RoughSetValue *rf)  // f == nullptr is treated as full filter
{
  LoadPackInfo();
  if (GetPackType() == common::PackType::STR)
...
uint64_t RCAttr::ApproxDistinctVals(bool incl_nulls, Filter *f, common::RoughSetValue *rf, bool outer_nulls_possible) {
  LoadPackInfo();
  uint64_t no_dist = 0;
  ...
  if (TypeName() == common::ColumnType::DATE) {
    try {
      types::RCDateTime date_min(RoughMin(f, rf), common::ColumnType::DATE);
      types::RCDateTime date_max(RoughMax(f, rf), common::ColumnType::DATE);
    ...
    try {
      types::RCDateTime date_min(RoughMin(f, rf), common::ColumnType::YEAR);
      types::RCDateTime date_max(RoughMax(f, rf), common::ColumnType::YEAR);
