...
bool RecordMergeOperator::Merge(const rocksdb::Slice &key, const rocksdb::Slice *existing_value,
  ...
  if (existing_type == RecordType::kInsert) {
    ...
    if (type == RecordType::kUpdate) {
      ...
      DeltaRecordHeadForInsert insertRecord(e_insertRecord.is_deleted_, e_insertRecord.table_id_, e_insertRecord.table_path_,
                                            e_insertRecord.field_count_, n_load_num);
      n_ptr = insertRecord.record_encode(n_ptr);

      for (uint i = 0; i < updateRecord.field_count_; i++) {
        // resize
        size_t length = updateRecord.field_head_[i] > 0 ? updateRecord.field_head_[i] : e_insertRecord.field_head_[i] > 0
                                                                                        ? e_insertRecord.field_head_[i] : 0;
      ...
      ptr = deleteRecord.record_decode(ptr);
    ...
    if (type == RecordType::kUpdate) {
      ...
      for (uint i = 0; i < updateRecord.field_count_; i++) {
        // resize
        size_t length = updateRecord.field_head_[i] > 0 ? updateRecord.field_head_[i] : e_updateRecord.field_head_[i] > 0
                                                                                        ? e_updateRecord.field_head_[i] : 0;
        size_t used = n_ptr - value_buff.get();
        if (value_buff_size - used < length) {
          while (value_buff_size - used < length) {
            value_buff_size *= 2;
      ...
      std::memcpy(value_buff.get() + n_updateRecord.update_offset_,
                  updateRecord.update_mask_.data(),
