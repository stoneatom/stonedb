...
class MultiIndex {
 ...
 private:
  ...
  uint32_t num_of_dimensions_{0};
  int64_t *dim_size_{nullptr};  // the size of a dimension
  uint64_t num_of_tuples_{0};   // actual number of tuples (also in case of virtual index); should be updated in any
                                // change of index
  uint32_t p_power_{0};

  bool has_tuples_too_big_{false};  // this flag is set if a virtual number of tuples exceeds 2^64
  ...
  DimensionGroup **dimension_group_{nullptr};  // pointers to elements of dim_groups, for
