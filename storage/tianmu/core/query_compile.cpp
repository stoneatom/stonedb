...
QueryRouteTo Query::Compile(CompiledQuery *compiled_query, SELECT_LEX *selects_list, SELECT_LEX *last_distinct,
  ...
  for (SELECT_LEX *sl = selects_list; sl; sl = sl->next_select()) {
    ...
    */
    ...
    // partial optimization of LOJ conditions, JOIN::optimize(part=3) necessary due to already done basic transformation
    // of conditions see comments in sql_select.cc:JOIN::optimize()
    if (IsLOJ(join_list) &&
