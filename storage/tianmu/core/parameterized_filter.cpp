...
double ParameterizedFilter::EvaluateConditionNonJoinWeight(Descriptor &d, bool for_or) {
  // Interpretation of weight: an approximation of logarithm of answer size
  // (in case of for_or: an approximation of (full_table - answer) size)
  // 0 -> time is very short (constant). high weight -> schedule this query to be executed later
  double eval = 0.0;
  uint64_t no_distinct, no_distinct2, answer_size;
  ...
  if (d.IsTrue() || d.IsFalse())
    ...
    if (!d.encoded)
      ...
      eval = (for_or) ? log(1 + (no_obj - double(no_obj) / no_distinct))
                      : log(1 + double(no_obj) / no_distinct);  // size of the smaller abstract class
...
double ParameterizedFilter::EvaluateConditionJoinWeight(Descriptor &d) {
  ...
  */
  if (dim1 > -1 && dim2 > -1) {
    uint64_t no_obj1 =
        mind->OrigSize(dim1);  // changed to uint64_t to prevent negative logarithm for common::NULL_VALUE_64
    ...
    double r_select1 = 0.5 + 0.5 * d.attr.vc->RoughSelectivity();  // 1 if no rough exclusions,
                                                                   // close to 0.5 if good rough performance
    ...
    if (d.op == common::Operator::O_EQ) {
      ...
      if (no_distinct1 >= 0.99 * mind->DimSize(dim1)) {  // potentially 1:n join
        ...
        // no_dist2) where no_of_classes = min(no_dist1,no_dist2) ,  because they must have common values
...
bool ParameterizedFilter::RoughUpdateMultiIndex() {
  ...
  bool is_nonempty{true}, false_desc{false};
...
void ParameterizedFilter::DisplayJoinResults(DimensionVector &all_involved_dims, JoinAlgType join_performed,
  ...
  (conditions_used > 1) ? std::sprintf(buf, "%d cond. ", conditions_used) : std::sprintf(buf, "%s", "");

  (is_outer) ? std::strcat(buf, "outer ") : std::strcat(buf, "inner ");
  ...
  for (int i = 0; i < mind->NumOfDimensions(); i++) {
    ...
    if (first) {
      std::string fmt_str = (all_involved_dims.NoDimsUsed() == 1) ? "...-%d" : "%d";
      std::sprintf(buf_dims, fmt_str.c_str(), i);
      first = false;
    } else
      std::sprintf(buf_dims + std::strlen(buf_dims), "-%d", i);
  }
...
bool ParameterizedFilter::TryToMerge(Descriptor &d1, Descriptor &d2)  // true, if d2 is no longer needed
{
  ...
  //    WHERE a=b
  if (d1.EqualExceptOuter(d2) && d1.IsInner() && d2.IsOuter())  // delete d2
    return true;

  if (d1.EqualExceptOuter(d2) && d1.IsOuter() && d2.IsInner()) {  // delete d1
    d1 = d2;
    return true;
  }

  // Content-based merging
  if ((d1.attr.vc == d2.attr.vc) && d1.attr.vc->TryToMerge(d1, d2))
    return true;
...
void ParameterizedFilter::SyntacticalDescriptorListPreprocessing(bool for_rough_query) {
  ...
  if (outer_join_found) {
    for (uint i = 0; i < no_desc; i++) {
      ...
      if (descriptors[i].NullMayBeTrue()) {
        ...
        // all nulls are excluded anyway, so it is an equivalent of inner join
        all_outer.Minus(inner);  // set all involved dimensions as no longer outer
      }
    }

    for (uint j = 0; j < no_desc; j++) {
      if (descriptors[j].done || !descriptors[j].IsOuter() || all_outer.Intersects(descriptors[j].right_dims))
        continue;
  ...
  for (uint i = 0; i < no_desc; i++) {           // Note that desc.size() may enlarge when joining with BETWEEN occur
    ...
    if (descriptors[i].IsFalse() || descriptors[i].IsDelayed())
      continue;
    ...
    //	throw common::NotImplementedException("This kind of join condition with OR is not implemented.");
  ...
  for (uint i = 0; i < no_desc; i++) {  // t1.x == t2.y && t2.y == 5   =>   t1.x == 5
    if (descriptors[i].done || descriptors[i].op != common::Operator::O_EQ || !descriptors[i].IsType_JoinSimple() ||
        !descriptors[i].IsInner())  // desc[i] is a joining (eq.) condition
      continue;
...
void ParameterizedFilter::DescriptorListOrdering() {
  ...
  if (descriptors.Size() > 1 || descriptors[0].IsType_OrTree()) {  // else all evaluations remain 0 (default)
    for (uint i = 0; i < descriptors.Size(); i++) {
      descriptors[i].evaluation = (descriptors[i].done || descriptors[i].IsDelayed()) ? 100000 :  // to the end of queue
                                      EvaluateConditionNonJoinWeight(descriptors[i]);  // joins are evaluated separately
    }
  }
  ...
  for (uint k = 0; k < descriptors.Size(); k++) {
    for (uint i = 0; i < descriptors.Size() - 1; i++) {
      ...
      if (descriptors[i].evaluation < descriptors[j].evaluation || !descriptors[j].IsInner() ||
        ...
        continue;  // do not change outer join order
...
void ParameterizedFilter::UpdateMultiIndex(bool count_only, int64_t limit) {
  ...
  if (descriptors.Size() < 1) {
    ...
    return;
  } else { /*Judge whether there is filtering logic of the current table. If not, filter the data of the current table*/
    auto &rcTables = table->GetTables();
    /* no_dims is a destination number of dimensions. It needs to be increased according to the number of executions. */
  ...
  for (uint i = 0; i < descriptors.Size(); i++) {
    ...
    if (descriptors[i].IsType_Join() || descriptors[i].IsDelayed() || descriptors[i].IsOuter() ||
      ...
      (!descriptors[i].IsDelayed()) ? no_of_join_conditions++ : no_of_delayed_conditions++;
    }
  }
  ...
  for (int i = 0; i < mind->NumOfDimensions(); i++) {
    ...
    table->SetVCDistinctVals(i, mind->GetFilter(i)->NumOfOnes());  // distinct values - not more than the
...
void ParameterizedFilter::TaskProcessPacks(MIUpdatingIterator *taskIterator, Transaction *ci, common::RoughSetValue *rf,
  ...
  while (taskIterator->IsValid()) {
    ...
    if (rf && taskIterator->GetCurPackrow(one_dim) >= 0)
      cur_roughval = rf[taskIterator->GetCurPackrow(one_dim)];
    else
      cur_roughval = common::RoughSetValue::RS_SOME;
