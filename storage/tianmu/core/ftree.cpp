...
FTree::FTree(const FTree &ft)
  ...
  if (total_buf_size > 0 && (!mem || !len || !value_offset)) {
    Destroy();
    TIANMU_LOG(LogCtl_Level::ERROR, "FTree, out of memory.");
...
void FTree::Init(int width) {
  ...
  if (len == nullptr) len = (uint16_t *)alloc(total_dic_size * sizeof(uint16_t), mm::BLOCK_TYPE::BLOCK_TEMPORARY, true);
...
int FTree::Add(const char *str, size_t sz) {
  ...
  if (size_t(hdr.size) > tianmu_sysvar_lookup_max_size) {
    throw common::OutOfMemoryException("Too many lookup values. Dictionary size is " +
                                       std::to_string(tianmu_sysvar_lookup_max_size));
  }
  ...
  if (total_dic_size < size_t(hdr.size + 1)) {  // Enlarge tables, if required
    ...
    if (len == nullptr || value_offset == nullptr) throw common::OutOfMemoryException("Too many lookup values");
  ...
  if (hdr.size != 0) {
    ...
    if (TIANMU_LOGCHECK(LogCtl_Level::DEBUG)) {
      if (CheckConsistency() != 0) {
        TIANMU_LOG(LogCtl_Level::DEBUG, "FTree CheckConsistency fail");
      }
    }
  }
...
void FTree::LoadData(const fs::path &p) {
  ...
  if (hdr.ver != DICT_FILE_VERSION) {
    TIANMU_LOG(LogCtl_Level::ERROR, "bad dictionary version %d", hdr.ver);
    return;
  }
...
void FTree::InitHash() {
  ...
  if (hash_table == nullptr) throw common::OutOfMemoryException("Too many lookup values");
...
int FTree::HashFind(const char *v, int v_len, int position_if_not_found) {
  if (hash_table == nullptr) InitHash();
