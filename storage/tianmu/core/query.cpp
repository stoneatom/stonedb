...
bool Query::IsCountStar(Item *item_sum) {
  Item_sum_count *is = down_cast<Item_sum_count *>(item_sum);
  if (is)
...
bool Query::IsDeterministic(Item *item) {
  switch (static_cast<int>(item->type())) {
    case Item::FUNC_ITEM: {
      ...
      if ((down_cast<Item_func_rand *>(ifunc) || down_cast<Item_func_last_insert_id *>(ifunc) ||
                 down_cast<Item_func_get_system_var *>(ifunc) || down_cast<Item_func_is_free_lock *>(ifunc) ||
                 down_cast<Item_func_is_used_lock *>(ifunc) || down_cast<Item_func_row_count *>(ifunc) ||
                 down_cast<Item_func_sp *>(ifunc)
...
bool Query::HasAggregation(Item *item) {
  ...
  if (i->type() == Item::SUM_FUNC_ITEM)
    ...
    Item_func *f = down_cast<Item_func *>(i);
    ...
    Item_cond *c = down_cast<Item_cond *>(i);
    ...
    if (down_cast<Item_tianmufield *>(i)->IsAggregation())
      has = true;
...
bool Query::IsAggregationOverFieldItem(Item *item) {
  return IsAggregationItem(item) && IsFieldItem((down_cast<Item_sum *>(item))->get_arg(0));
}
...
const std::string Query::GetItemName(Item *item) {
  ...
  switch (static_cast<int>(item->type())) {
    ...
    case Item::REF_ITEM: {
      Item_ref *ref = down_cast<Item_ref *>(item);
    ...
    case static_cast<int>(Item_tianmufield::enumTIANMUFiledItem::TIANMUFIELD_ITEM):
      Item_tianmufield *tianmu = down_cast<Item_tianmufield *>(item);
...
void Query::ExtractOperatorType(Item *&conds, common::Operator &op, bool &negative, char &like_esc) {
  ...
  Item_func *cond_func = down_cast<Item_func *>(conds);
  switch (cond_func->functype()) {
    case Item_func::BETWEEN:  // between
      op = is_there_not ? common::Operator::O_NOT_BETWEEN : common::Operator::O_BETWEEN;
      break;
    case Item_func::LIKE_FUNC:  // like
      op = is_there_not ? common::Operator::O_NOT_LIKE : common::Operator::O_LIKE;
      like_esc = down_cast<Item_func_like *>(cond_func)->escape;
      break;
    case Item_func::ISNULL_FUNC:  // is null
      op = common::Operator::O_IS_NULL;
      break;
    case Item_func::ISNOTNULL_FUNC:  // is not null
      op = common::Operator::O_NOT_NULL;
      break;
    case Item_func::IN_FUNC:  // in
    ...
    case Item_func::LT_FUNC:  // <
      ...
      break;
    case Item_func::MULT_EQUAL_FUNC:  // a=b=c
      op = common::Operator::O_MULT_EQUAL_FUNC;
      break;
    case Item_func::NOT_FUNC:  // not
      op = common::Operator::O_NOT_FUNC;
      break;
    case Item_func::NOT_ALL_FUNC: {
      Item_func *cond_func = down_cast<Item_func *>(conds);
      negative = down_cast<Item_func_nop_all *>(cond_func) == nullptr;
      ExtractOperatorType(cond_func->arguments()[0], op, negative, like_esc);

      if (down_cast<Item_func_nop_all *>(cond_func))
        MarkWithAny(op);
      else if (down_cast<Item_func_not_all *>(cond_func))
        MarkWithAll(op);
...
TempTable *Query::Preexecute(CompiledQuery &qu, ResultSender *sender, [[maybe_unused]] bool display_now) {
  ...
  for (int i = 0; i < qu.NumOfSteps(); i++) {
    ...
    if (step.alias && std::strcmp(step.alias, "roughstats") == 0) {
      ...
      dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->DisplayRSI();
    }
    ...
    try {
      switch (step.type) {
        ...
        case CompiledQuery::StepType::TMP_TABLE:
          ...
          dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->ReserveVirtColumns(qu.NumOfVirtualColumns(step.t1));
          break;
        case CompiledQuery::StepType::CREATE_CONDS:
          DEBUG_ASSERT(step.t1.n < 0);
          step.e1.vc = (step.e1.vc_id != common::NULL_VALUE_32)
                           ? dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->GetVirtualColumn(step.e1.vc_id)
                           : nullptr;
          step.e2.vc = (step.e2.vc_id != common::NULL_VALUE_32)
                           ? dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->GetVirtualColumn(step.e2.vc_id)
                           : nullptr;
          step.e3.vc = (step.e3.vc_id != common::NULL_VALUE_32)
                           ? dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->GetVirtualColumn(step.e3.vc_id)
                           : nullptr;
          ...
          if (step.n1 != static_cast<int64_t>(CondType::OR_SUBTREE)) {  // on result = false
            ...
            if (step.c2.IsNull()) {
              conds[step.c1.n]->AddDescriptor(
                  step.e1, step.op, step.e2, step.e3, dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get()),
              ...
              conds[step.c1.n]->AddDescriptor(static_cast<SingleTreeCondition *>(conds[step.c2.n])->GetTree(),
                                              dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get()),
                                              qu.GetNumOfDimens(step.t1));
            }
          } else {  // on result = true
            if (step.c2.IsNull())
              conds[step.c1.n] = new SingleTreeCondition(step.e1, step.op, step.e2, step.e3,
                                                         dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get()),
                                                         qu.GetNumOfDimens(step.t1), char(step.n2));
            else {
              DEBUG_ASSERT(conds[step.c2.n]->IsType_Tree());
              conds[step.c1.n] = new Condition();
              conds[step.c1.n]->AddDescriptor((dynamic_cast<SingleTreeCondition *>(conds[step.c2.n])->GetTree()),
                                              dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get()),
                                              qu.GetNumOfDimens(step.t1));
            }
          }
          ...
          if (!conds[step.c2.n]->IsType_Tree()) {
            ...
            for (size_t i = 0; i < cond2->Size(); i++) {
              ...
              if (conds[step.c1.n]->IsType_Tree()) {
                TempTable *temptb = dynamic_cast<TempTable *>(ta[-qu.GetTableOfCond(step.c2).n - 1].get());
            ...
            DEBUG_ASSERT(conds[step.c2.n]->IsType_Tree());
            conds[step.c1.n]->AddDescriptor(static_cast<SingleTreeCondition *>(conds[step.c2.n])->GetTree(),
                                            dynamic_cast<TempTable *>(ta[-qu.GetTableOfCond(step.c1).n - 1].get()),
        ...
        case CompiledQuery::StepType::AND_DESC: {
          ...
          if (!conds[step.c1.n]->IsType_Tree()) {
            ...
            static_cast<SingleTreeCondition *>(conds[step.c1.n])
                ->AddDescriptor(lop, step.e1, step.op, step.e2, step.e3,
                                dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get()), qu.GetNumOfDimens(step.t1),
        ...
        case CompiledQuery::StepType::T_MODE:
          ...
          dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->SetMode(step.tmpar, step.n1, step.n2);
          break;
        case CompiledQuery::StepType::JOIN_T:
          DEBUG_ASSERT(step.t1.n < 0 && ta[-step.t1.n - 1]->TableType() == TType::TEMP_TABLE);
          dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->JoinT(t2_ptr.get(), step.t2.n, step.jt);
          break;
        case CompiledQuery::StepType::ADD_CONDS: {
          ...
          dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->AddConds(conds[step.c1.n], (CondType)step.n1);
          break;
        }
        case CompiledQuery::StepType::LEFT_JOIN_ON: {
          ...
          dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
              ->AddLeftConds(conds[step.c1.n], step.tables1, step.tables2);
          break;
        }
        case CompiledQuery::StepType::INNER_JOIN_ON: {
          ...
          dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->AddInnerConds(conds[step.c1.n], step.tables1);
          break;
        }
        case CompiledQuery::StepType::APPLY_CONDS: {
          int64_t cur_limit = -1;
          auto *tb = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get());
        ...
        case CompiledQuery::StepType::ADD_COLUMN: {
          ...
          if (e.vc_id != common::NULL_VALUE_32)
            e.vc = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
                       ->GetVirtualColumn(step.e1.vc_id);  // vc must have been created
          step.a1.n = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
        ...
        case CompiledQuery::StepType::CREATE_VC: {
          ...
          TempTable *t = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get());
          DEBUG_ASSERT(t);
          ...
          if (step.mysql_expr.size() > 0) {
            ...
            int c = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
            ...
            if (step.a2.n != common::NULL_VALUE_32)
              ct = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->GetVirtualColumn(step.a2.n)->Type();

            std::vector<vcolumn::VirtualColumn *> vcs;
            for (uint i = 0; i < step.virt_cols.size(); i++)
              vcs.push_back(dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->GetVirtualColumn(step.virt_cols[i]));
            ...
            PhysicalColumn *phc{nullptr};

            MultiIndex *mind = (step.t2.n == step.t1.n) ? t->GetOutputMultiIndexP() : t->GetMultiIndexP();
            int dim = (step.t2.n == step.t1.n) ? 0 : t->GetDimension(step.t2);
            phc = dynamic_cast<PhysicalColumn *>(t_src->GetColumn(step.a2.n >= 0 ? step.a2.n : -step.a2.n - 1));
            int c = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
                        ->AddVirtColumn(
        ...
        case CompiledQuery::StepType::ADD_ORDER: {
          ...
          TempTable *loc_t = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get());
        ...
        case CompiledQuery::StepType::UNION:
          ...
          if (step.t1.n != step.t2.n)
            ta[-step.t1.n - 1] = TempTable::Create(*dynamic_cast<TempTable *>(ta[-step.t2.n - 1].get()), false);
          ...
          if (IsRoughQuery()) {
            if (step.t3.n == common::NULL_VALUE_32)
              ...
              dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
                  ->RoughUnion(dynamic_cast<TempTable *>(ta[-step.t3.n - 1].get()),
                               qu.IsResultTable(step.t1) ? sender : nullptr);
          } else if (qu.IsResultTable(step.t1) && !qu.IsOrderedBy(step.t1) && step.n1)
            dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
                ->Union(dynamic_cast<TempTable *>(ta[-step.t3.n - 1].get()), (int)step.n1, sender, global_limits.first,
                        global_limits.second);
          else {
            if (step.t3.n == common::NULL_VALUE_32)
              dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())->Union(nullptr, (int)step.n1);
            else {
              dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get())
                  ->Union(dynamic_cast<TempTable *>(ta[-step.t3.n - 1].get()), (int)step.n1);
        ...
        case CompiledQuery::StepType::RESULT:
          ...
          output_table = dynamic_cast<TempTable *>(ta[-step.t1.n - 1].get());
          break;
...
QueryRouteTo Query::Item2CQTerm(Item *an_arg, CQTerm &term, const TabID &tmp_table, CondType filter_type, bool negative,
  ...
  if (an_arg->type() == Item::SUBSELECT_ITEM) {
    Item_subselect *item_subs = down_cast<Item_subselect *>(an_arg);
...
Item *Query::FindOutAboutNot(Item *it, bool &is_there_not) {
  ...
  Item_func *func = down_cast<Item_func *>(it);
  /*Then we remove negation in case of LIKE (or sth. else)*/
  if (func && func->functype() == Item_func::NOT_FUNC) {
    Item *arg = UnRef(func->arguments()[0]);
    ...
    // down to LIKE
    if (arg->type() == Item::FUNC_ITEM && down_cast<Item_func *>(arg)->functype() == Item_func::LIKE_FUNC) {
      is_there_not = true;
      return arg;
    }
  }
...
CondID Query::ConditionNumberFromComparison(Item *conds, const TabID &tmp_table, CondType filter_type,
  ...
  if (op == common::Operator::O_MULT_EQUAL_FUNC)
    return ConditionNumberFromMultipleEquality(down_cast<Item_equal *>(conds), tmp_table, filter_type, and_me_filter,
                                               is_or_subtree);
...
TableStatus Query::PrefixCheck(Item *conds) {
  ...
  switch (cond_type) {
    case Item::COND_ITEM: {
      Item_cond *cond_cond = down_cast<Item_cond *>(conds);
      Item_func::Functype func_type = cond_cond->functype();
