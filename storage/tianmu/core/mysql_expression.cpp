...
bool generic_item_same(Item const &left, Item const &right) {
  return ((&left == &right) || ((left.type() == right.type()) && (left.result_type() == right.result_type()) &&
                                (left.cast_to_int_type() == right.cast_to_int_type()) &&
                                (left.string_field_type(left.max_length) == right.string_field_type(left.max_length)) &&
                                (left.data_type() == right.data_type()) &&
                                (left.const_for_execution() == right.const_for_execution()) && (left == right)));
}
}  // namespace
      ...
      case (Item::FUNC_ITEM): {
        ...
        if (l->functype() == Item_func::GUSERVAR_FUNC) {
          ...
          if (same && l->const_item())
            same = ((dynamic_cast<Item_func *>(const_cast<Item *>(&left)))->val_int() ==
                    (dynamic_cast<Item_func *>(const_cast<Item *>(&right)))->val_int());
