...
common::RoughSetValue RSIndex_Hist::IsValue(int64_t min_v, int64_t max_v, int pack, int64_t pack_min,
  ...
  if (!Fixed()) {  // floating point
    ...
    // common::RoughSetValue::RS_SOME or common::RoughSetValue::RS_NONE answer possible
...
void RSIndex_Hist::Update(common::PACK_INDEX pi, DPN &dpn, const PackInt *pack) {
  ...
  double interval_len =
      Fixed() ? (dpn.max_i - dpn.min_i) / double(RSI_HIST_BITS) : (dpn.max_d - dpn.min_d) / double(RSI_HIST_BITS);
