...
DROP TABLE IF EXISTS sys_tianmu.dummy;
CREATE TABLE sys_tianmu.dummy (dummy_int int) ENGINE=tianmu;

DROP PROCEDURE IF EXISTS sys_tianmu_wait;
CREATE DEFINER=root@localhost PROCEDURE sys_tianmu_wait() BEGIN SELECT COUNT(*) INTO @num_rec FROM sys_tianmu.dummy;WHILE (@num_rec != 1) DO SELECT COUNT(*) INTO @num_rec FROM sys_tianmu.dummy; SELECT SLEEP(1) INTO @sleep;END WHILE;END;
INSERT INTO sys_tianmu.dummy values (0);

CALL sys_tianmu_wait();
