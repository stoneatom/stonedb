...
JOIN::optimize(unsigned char part)
{
  ...
  */
  conds= optimize_cond(thd, conds, &cond_equal,
                       join_list, true, &select_lex->cond_value, part);
  if (thd->is_error())
  {
    error= 1;
  ...
  */
  if (having && const_table_map && !having->with_sum_func)
  {
    ...
    having= remove_eq_conds(thd, having, &select_lex->having_value, part);
                         ...
                         unsigned char part)
{
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    *cond_value=Item::COND_UNDEF;
    ...
    while ((item=li++))
    {
      Item *new_item=internal_remove_eq_conds(thd, item, &tmp_cond_value, part);
      if (!new_item)
	li.remove();
...
Item *
remove_eq_conds(THD *thd, Item *cond, Item::cond_result *cond_value, unsigned char part)
{
  if (cond->type() == Item::FUNC_ITEM &&
      ((Item_func*) cond)->functype() == Item_func::ISNULL_FUNC)
  {
  ...
  return internal_remove_eq_conds(thd, cond, cond_value, part); // Scan all the condition
}
