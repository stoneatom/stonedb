...
void JOIN::reset()
{
  ...
  unit->offset_limit_cnt = (ha_rows)(select_lex->offset_limit ? select_lex->offset_limit->val_uint() : 0ULL);
              ...
              KEY_PART_INFO *key_part, uchar *key_buff, uint maybe_null)
{
  ...
  else if (keyuse->val->type() == Item::REF_ITEM)
  {
    ...
    if (item_ref->ref_type() == Item_ref::OUTER_REF)
    {
      ...
      else if ((*(Item_ref **)(item_ref)->ref)->ref_type() == Item_ref::DIRECT_REF &&
                                             ...
                                             POSITION *sjm_pos)
{
  ...
  */
  ...
  double fanout = ((uint)tab->idx() == const_tables) ? 1.0 : best_ref[tab->idx() - 1]->position()->prefix_rowcount;
  if (!sjm_exec->is_scan)
  {
  ...
  for (uint i = join->const_tables; i < join->tables; i++)
  {
    ...
    */
    ...
    if (tab->position()->filter_effect <= COND_FILTER_STALE)
    {
      ...
      */
      tab->position()->filter_effect =
          join->thd->lex->describe ? calculate_condition_filter(tab,
        ...
        qep_tab[curr_tmp_table].type() != JT_EQ_REF) // Don't sort 1 row
    {
      ...
      */
      sort_tab->filesort->limit =
          (has_group_by || (primary_tables > curr_tmp_table + 1)) ? m_select_limit : unit->select_limit_cnt;
    }
    if (!plan_is_const() &&
    ...
    if (tab->ref().key >= 0)
    {
      ...
      else
      {
        ...
        */
        ...
        QUICK_SELECT_I *q = tab->type() == JT_FT ? get_ft_select(thd, table, tab->ref().key) : get_quick_select_for_ref(thd, table, &tab->ref(), jtab->found_records);
        if (!q)
...
bool test_if_cheaper_ordering(const JOIN_TAB *tab, ORDER *order, TABLE *table,
  ...
  if (select_limit >= table_records)
  {
    ...
    */
    if (table->force_index)
      keys.merge(group ? table->keys_in_use_for_group_by : table->keys_in_use_for_order_by);
        ...
        (direction = test_if_order_by_key(order, table, nr, &used_key_parts)))
    {
      ...
      */
          ...
          (ref_key < 0 && (group || table->force_index)))
      {
        ...
        */
        select_limit = (ha_rows)(select_limit < fanout ? 1 : select_limit / fanout);
        /*
        ...
        */
        if (((cur_access_method == JT_ALL || cur_access_method == JT_INDEX_SCAN) && (is_covering || group || table->force_index)) ||
            index_scan_time < read_time)
        {
          ha_rows quick_records = table_records;
          const ha_rows refkey_select_limit =
              (ref_key >= 0 && table->covering_keys.is_set(ref_key)) ? static_cast<ha_rows>(refkey_rows_estimate) : HA_POS_ERROR;
          ...
          if (best_key < 0 ||
              (select_limit <= min(quick_records, best_records) ? keyinfo->user_defined_key_parts < best_key_parts : quick_records < best_records))
          {
            best_key = nr;
...
{
  return key_info->table->in_use->optimizer_switch_flag(OPTIMIZER_SWITCH_USE_INDEX_EXTENSIONS) ? key_info->actual_key_parts : key_info->user_defined_key_parts;
}
...
{
  return key_info->table->in_use->optimizer_switch_flag(OPTIMIZER_SWITCH_USE_INDEX_EXTENSIONS) ? key_info->actual_flags : key_info->flags;
}
