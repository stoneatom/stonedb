...
bool ignore_db_dirs_process_additions()
{
  ...
  if (my_hash_init(&ignore_db_dirs_hash,
                   lower_case_table_names ? character_set_filesystem : &my_charset_bin,
...
#endif
    if (!(file_name = tmp_mem_root ? make_lex_string_root(tmp_mem_root, file_name, uname,
                                                          file_name_len, TRUE)
...
public:
  ...
  */
  explicit Show_create_error_handler(THD *thd, TABLE_LIST *top_view) : m_top_view(top_view), m_handling(false),
                                                                       m_view_access_denied_message_ptr(NULL)
  {
    m_sctx = MY_TEST(m_top_view->security_ctx) ? m_top_view->security_ctx : thd->security_context();
  }
...
bool mysqld_show_create(THD *thd, TABLE_LIST *table_list)
{
  ...
  */
  ...
  if ((table_list->is_view() ? view_store_create_info(thd, table_list, &buffer) : store_create_info(thd, table_list, &buffer, NULL, FALSE /* show_database */)))
    goto exit;
  ...
  if (!(thd->variables.sql_mode & MODE_NO_TABLE_OPTIONS) && !foreign_db_mode)
  {
    ...
    */
    if (thd->variables.show_create_table_verbosity)
    {
      ...
      {
        row_type = ((share->db_options_in_use & HA_OPTION_COMPRESS_RECORD) ? ROW_TYPE_COMPRESSED : (share->db_options_in_use & HA_OPTION_PACK_RECORD) ? ROW_TYPE_DYNAMIC
                                                                                                                                                      : ROW_TYPE_FIXED);
      }
  ...
  */
  table->view_query()->unit->print(buff,
                                   enum_query_type(QT_TO_ARGUMENT_CHARSET |
                                                   (compact_view_format ? QT_NO_DB : 0)));
...
public:
  List_process_list(const char *user_value, Thread_info_array *thread_infos,
                    THD *thd_value, size_t max_query_length) : m_user(user_value), m_thread_infos(thread_infos),
  ...
  virtual void operator()(THD *inspect_thd)
  {
        ...
        m_client_thd->security_context()->host_or_ip().str[0])
    {
      ...
      thd_info->host =
          m_client_thd->mem_strdup(inspect_sctx_host_or_ip.str[0] ? inspect_sctx_host_or_ip.str : inspect_sctx_host.length ? inspect_sctx_host.str
                                                                                                                           : "");
...
void mysqld_list_processes(THD *thd, const char *user, bool verbose)
{
  ...
  size_t max_query_length = (verbose ? thd->variables.max_allowed_packet : PROCESS_LIST_WIDTH);
  Protocol *protocol = thd->get_protocol();
...
public:
  ...
  virtual void operator()(THD *inspect_thd)
  {
    ...
    const char *user =
        m_client_thd->security_context()->check_access(PROCESS_ACL) ? NullS : client_priv_user;
    ...
    if (inspect_thd->start_time.tv_sec)
      table->field[5]->store((longlong)(my_time(0) - inspect_thd->start_time.tv_sec), false);
    else
...
class Show_var_cmp : public std::binary_function<const st_mysql_show_var &,
  ...
  while ((db_name = it++))
  {
...
#endif
    {
      ...
      while ((table_name = it_files++))
      {
        assert(table_name->length <= NAME_LEN);
        restore_record(table, s->default_values);
        table->field[schema_table->idx_field1]->store(db_name->str, db_name->length, system_charset_info);
        table->field[schema_table->idx_field2]->store(table_name->str, table_name->length, system_charset_info);
                       ...
                       uint offset)
{
  ...
  */
  ...
  table->field[offset]->store(column_type.ptr(),
                              (tmp_buff ? tmp_buff - column_type.ptr() : column_type.length()), cs);
      ...
      field->real_type() == MYSQL_TYPE_STRING)    // For binary type
  {
    ...
    longlong char_max_len = is_blob ? (longlong)octet_max_length / field->charset()->mbminlen : (longlong)octet_max_length / field->charset()->mbmaxlen;
                                    ...
                                    LEX_STRING *table_name)
{
  ...
  for (; (field = *ptr); ptr++)
  {
    ...
    pos = (uchar *)((field->flags & PRI_KEY_FLAG) ? "PRI" : (field->flags & UNIQUE_KEY_FLAG) ? "UNI"
    ...
    {
      if (field->stored_in_db)
        table->field[IS_COLUMNS_EXTRA]->store(STRING_WITH_LEN("STORED GENERATED"), cs);
      else
        table->field[IS_COLUMNS_EXTRA]->store(STRING_WITH_LEN("VIRTUAL GENERATED"), cs);
      table->field[IS_COLUMNS_GENERATION_EXPRESSION]->store(field->gcol_info->expr_str.str, field->gcol_info->expr_str.length,
                                                            cs);
    }
    else
                         ...
                         const char *sp_user)
{
  ...
  if (sp)
  {
    ...
    {
      restore_record(table, s->default_values);
      table->field[IS_PARAMETERS_SPECIFIC_CATALOG]->store(STRING_WITH_LEN("def"), cs);
                       ...
                       const char *wild, bool full_access, const char *sp_user)
{
  ...
  {
    restore_record(table, s->default_values);
    if (!wild || !wild[0] || !wild_case_compare(system_charset_info, sp_name.c_ptr_safe(), wild))
    {
      ...
      table->field[IS_ROUTINES_SQL_DATA_ACCESS]->store(sp_data_access_name[enum_idx].str,
...
int fill_schema_proc(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  if (schema_table_idx == SCH_PROCEDURES ? store_schema_proc(thd, table, proc_table, wild, full_access, definer) : store_schema_params(thd, table, proc_table, wild, full_access, definer))
  {
    res = 1;
    goto err;
  }
  ...
  if (tables->is_view())
  {
    ...
    */
    ...
    table->field[8]->store(tables->view_creation_ctx->get_client_cs()->csname,
                           strlen(tables->view_creation_ctx->get_client_cs()->csname), cs);

    table->field[9]->store(tables->view_creation_ctx->get_connection_cl()->name,
                           strlen(tables->view_creation_ctx->get_connection_cl()->name), cs);
...
int copy_event_to_schema_table(THD *thd, TABLE *sch_table, TABLE *event_table)
{
  ...
  */
  ...
  sch_table->field[ISE_EVENT_SCHEMA]->store(et.dbname.str, et.dbname.length, scs);
  sch_table->field[ISE_EVENT_NAME]->store(et.name.str, et.name.length, scs);
  sch_table->field[ISE_DEFINER]->store(et.definer.str, et.definer.length, scs);
  const String *tz_name = et.time_zone->get_name();
  sch_table->field[ISE_TIME_ZONE]->store(tz_name->ptr(), tz_name->length(), scs);
  ...
  if (et.expression)
  {
    ...
    sch_table->field[ISE_INTERVAL_VALUE]->store(show_str.ptr(), show_str.length(), scs);
  ...
  if (et.on_completion == Event_parse_data::ON_COMPLETION_DROP)
    sch_table->field[ISE_ON_COMPLETION]->store(STRING_WITH_LEN("NOT PRESERVE"), scs);
  else
    sch_table->field[ISE_ON_COMPLETION]->store(STRING_WITH_LEN("PRESERVE"), scs);
  ...
  sch_table->field[ISE_EVENT_COMMENT]->store(et.comment.str, et.comment.length, scs);
...
int mysql_schema_table(THD *thd, LEX *lex, TABLE_LIST *table_list)
{
  ...
  */
  ...
  if (table_list->schema_table_reformed) // show command
  {
    ...
    if (!(transl =
              (Field_translator *)(thd->stmt_arena->alloc(sel->item_list.elements *
