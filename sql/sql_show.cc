...
#include "../storage/stonedb/handler/ha_rcengine.h"
				    ...
				    LEX_STRING *table_name)
{
  ...
  for (; (field= *ptr) ; ptr++)
  {
    ...
    //table->field[IS_COLUMNS_COLUMN_COMMENT]->store(field->comment.str, field->comment.length, cs);
    stonedb::dbhandler::SDB_UpdateAndStoreColumnComment(table, IS_COLUMNS_COLUMN_COMMENT, field, count - 1, cs);
