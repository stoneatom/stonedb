  ...
  */
  ...
  Execute_load_query_log_event
      e(thd, load_data_query, end - load_data_query,
        ...
        (duplicates == DUP_REPLACE) ? binary_log::LOAD_DUP_REPLACE : (thd->lex->is_ignore() ? binary_log::LOAD_DUP_IGNORE : binary_log::LOAD_DUP_ERROR),
  ...
  for (;; it.rewind())
  {
    ...
    */
    ...
    while ((item = it++))
    {
      ...
      {
        assert(NULL != dynamic_cast<Item_user_var_as_out_param *>(item));
        ((Item_user_var_as_out_param *)item)->set_value((char *)tag->value.ptr(), tag->value.length(), cs);
      }
    }
  ...
  */
  ...
  else
  {
    ...
    if (init_io_cache(&cache, (get_it_from_net) ? -1 : file, 0,
                      (get_it_from_net) ? READ_NET : (is_fifo ? READ_FIFO : READ_CACHE), 0L, 1,
                      MYF(MY_WME)))
    {
