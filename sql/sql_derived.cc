  ...
  if (lex->derived_tables)
  {
         ...
         sl= sl->next_select_in_list())
    {
           ...
           table_ref= table_ref->next_local)
      {
        if (derived_optimized)
        {
          // Checks whether the derived table was optimized by StoneDB
          bh_it.rewind();
          st_select_lex_unit* found;
          while ((found = bh_it++))
            if (found==table_ref->derived) break;
          if (found && (res = table_ref->derived->optimize_after_stonedb()))
            goto out;
        }
